...
private fun App(repo: Repositorio, auth: Autenticacao, config: ConfiguracoesApp) {
    ...
    var papel by remember { mutableStateOf("jogador") }
    ...
    if (!logado) {
        LoginScreen(auth) { novoPapel ->
            papel = novoPapel
            logado = true
            repo.iniciarSincronizacao()
        }
        return
    }
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            ...
            if (criacao != null) {
                ...
                androidx.compose.foundation.layout.Box(m) { FichaScreen(repo, id) }
            } else if (pagina != null) {
