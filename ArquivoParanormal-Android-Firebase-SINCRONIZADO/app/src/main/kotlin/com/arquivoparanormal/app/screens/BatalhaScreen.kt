...
private fun IniciativaScreen(repo: Repositorio) {
    ...
    val ordem = repo.combatentes.sortedWith(compareByDescending<Combatente> { it.iniciativa }.thenBy { it.nome.lowercase() }.thenBy { it.id })
    ...
    LazyColumn(
        ...
        item {
            Painel(
                ...
                if (ordem.isNotEmpty()) {
                    Text(
                        "Ordem: ${ordem.joinToString("  •  ") { "${it.nome.ifBlank { "Combatente" }} (${it.iniciativa})" }}",
                        color = TextoFraco,
                ...
                if (mestre) {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Chip("Rolar iniciativas", false, { repo.rolarIniciativas() }, Primaria)
