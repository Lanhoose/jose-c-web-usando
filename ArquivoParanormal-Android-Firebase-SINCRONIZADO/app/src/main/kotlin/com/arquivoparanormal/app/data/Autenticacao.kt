        ...
        db.collection("fichas").whereEqualTo("ownerUid", uid).get()
            .addOnSuccessListener { snapshot ->
                ...
                snapshot.documents.forEach { doc ->
                    // A foto é um campo derivado. Use uma transação para ler a
                    // versão mais recente da ficha antes de escrever somente a
                    // miniatura. Assim, uma alteração de perícia/PV feita ao
                    // mesmo tempo não é sobrescrita por uma cópia antiga.
                    db.runTransaction { tx ->
                        val atualDoc = tx.get(doc.reference)
                        val payload = atualDoc.getString("payload") ?: return@runTransaction null
                        val personagem = runCatching {
                            kotlinx.serialization.json.Json { ignoreUnknownKeys = true; encodeDefaults = true }
                                .decodeFromString<Personagem>(payload)
                        }.getOrNull() ?: return@runTransaction null
                        if (personagem.fotoJogadorThumb == miniatura) return@runTransaction null
                        val atualizado = personagem.copy(fotoJogadorThumb = miniatura, fotoSolicitadaEm = 0L)
                        tx.set(
                            doc.reference,
                            mapOf(
                        ...
                        if (primeiroErro == null) primeiroErro = erro.message
