...
fun patentePorPrestigio(prestigio: Int): String = when {
    prestigio >= 200 -> "Agente de Elite"
    prestigio >= 100 -> "Oficial de Operações"
    prestigio >= 50 -> "Agente Especial"
    prestigio >= 20 -> "Operador"
    else -> "Recruta"
}

fun limitesItensPorCategoria(prestigio: Int): Map<Int, Int> = when {
    prestigio >= 200 -> mapOf(1 to 3, 2 to 3, 3 to 3, 4 to 2)
    prestigio >= 100 -> mapOf(1 to 3, 2 to 3, 3 to 2, 4 to 1)
    prestigio >= 50 -> mapOf(1 to 3, 2 to 2, 3 to 1, 4 to 0)
    prestigio >= 20 -> mapOf(1 to 3, 2 to 1, 3 to 0, 4 to 0)
    else -> mapOf(1 to 2, 2 to 0, 3 to 0, 4 to 0)
}

/** Perícias que o Combatente pode escolher na criação; as demais são livres para as outras classes. */
fun periciasDisponiveisParaClasse(classe: String): Set<String> = when (classe) {
    "Combatente" -> setOf("Atletismo", "Fortitude", "Luta", "Pontaria", "Reflexos", "Vontade")
    else -> PERICIAS.map { it.nome }.toSet()
}

fun periciasClasseCumpreRegra(classe: String, escolhidas: Set<String>): Boolean =
    classe != "Combatente" || escolhidas.any { it == "Luta" || it == "Pontaria" }
...
/** Tipos de dano do livro, incluindo os subtipos paranormais. */
val RESISTENCIAS = listOf(
    "Balístico", "Corte", "Impacto", "Perfuração",
    "Eletricidade", "Fogo", "Frio", "Químico",
    "Mental", "Paranormal", "Sangue", "Morte", "Conhecimento", "Energia", "Medo",
)

/** Proteções básicas. A Proteção Pesada fornece RD geral 2, isto é,
 * reduz qualquer tipo de dano. Isso é diferente de fontes que especificam
 * apenas um ou mais tipos de dano. */
data class ProtecaoDef(
    val nome: String,
    val categoria: Int,
    val espacos: Double,
    val defesaBonus: Int,
    val resistenciaGeral: Int,
)

val PROTECOES_LIVRO = listOf(
    ProtecaoDef("Proteção Leve", 1, 2.0, 5, 0),
    ProtecaoDef("Proteção Pesada", 2, 5.0, 10, 2),
)


data class ItemCatalogoDef(
    val nome: String,
    val categoria: Int = 2,
    val espacos: Double = 1.0,
    val tipoInventario: String = "Itens amaldiçoados",
    val descricao: String,
    val resistencias: Map<String, Int> = emptyMap(),
    val vulnerabilidades: Map<String, Int> = emptyMap(),
    val unico: Boolean = false,
    val elemento: String = "",
    val livro: String = "Livro de Regras",
    val pagina: Int = 0,
)

/** Itens amaldiçoados especiais catalogados individualmente a partir do Livro de Regras v1.3. */
val ITENS_AMALDICOADOS_LIVRO = listOf(
    ItemCatalogoDef("Coração Pulsante", descricao = "Permite reduzir pela metade um dano recebido ao custo de uma reação; cada uso exige Fortitude e pode destruir o item." , elemento="Sangue", pagina=147),
    ItemCatalogoDef("Coroa de Espinhos", descricao = "Uma vez por rodada, pode converter dano mental sofrido em dano de Sangue; enquanto vestida, impede recuperação de Sanidade por descanso. Ativa após uma semana.", elemento="Sangue", pagina=147),
    ItemCatalogoDef("Frasco de Vitalidade", descricao = "Armazena parte dos próprios PV e depois permite recuperá-los ao beber o conteúdo, com teste de Fortitude para evitar enjoo.", elemento="Sangue", pagina=147),
    ItemCatalogoDef("Pérola de Sangue", descricao = "Ao ser absorvida, concede +5 em testes de Agilidade, Força e Vigor até o fim da cena; depois exige Fortitude e pode causar efeitos graves.", elemento="Sangue", pagina=148),
    ItemCatalogoDef("Punhos Enraivecidos", descricao = "Soqueiras de Sangue: ataques desarmados causam +1d8 de Sangue, tornam-se letais e permitem ataques extras progressivos por PE.", elemento="Sangue", pagina=148),
    ItemCatalogoDef("Seringa de Transfiguração", descricao = "Extrai sangue de um alvo e permite injetá-lo em outro para alterar sua aparência por um dia, com possível consequência permanente ao retornar.", elemento="Sangue", pagina=148),
    ItemCatalogoDef("Amarras Mortais", descricao = "Correntes que permitem agarrar alvos Grandes ou menores em alcance curto com +10 no teste e puxar um alvo agarrado para perto.", elemento="Morte", pagina=148),
    ItemCatalogoDef("Casaco de Lodo", descricao = "Vestimenta de Lodo que concede RD 5 a Corte, Impacto, Perfuração e Morte, mas causa vulnerabilidade a Balístico e Energia.", resistencias=mapOf("Corte" to 5, "Impacto" to 5, "Perfuração" to 5, "Morte" to 5), vulnerabilidades=mapOf("Balístico" to 100, "Energia" to 100), elemento="Morte", pagina=149),
    ItemCatalogoDef("Coletora", descricao = "Punhal que pode executar uma criatura Morrendo e armazenar PE; o uso prolongado provoca pesadelos e descanso ruim.", elemento="Morte", pagina=149),
    ItemCatalogoDef("Crânio Espiral", descricao = "Pode conceder uma ação padrão adicional uma vez por rodada, mas cada uso exige Vontade e pode envelhecer o usuário.", elemento="Morte", pagina=149),
    ItemCatalogoDef("Frasco de Lodo", descricao = "Lodo de Morte que pode curar ferimentos recentes; em ferimentos antigos pode curar ou causar dano de Morte. Uso único.", elemento="Morte", pagina=149),
    ItemCatalogoDef("Vislumbre do Fim", descricao = "Permite descobrir informações sobre a morte de um ser, incluindo o pior valor de resistência e vulnerabilidades de criaturas.", elemento="Morte", pagina=149),
    ItemCatalogoDef("Anéis do Elo Mental", descricao = "Após 24 horas de uso por duas pessoas, cria uma ligação telepática que compartilha o melhor teste de Vontade e também compartilha dano mental e condições mentais/medo.", elemento="Conhecimento", pagina=149),
    ItemCatalogoDef("Lanterna Reveladora", descricao = "Quando ativada, emite luz com propriedades do Terceiro Olho e atrai criaturas de Sangue que forem iluminadas.", elemento="Conhecimento", pagina=149),
    ItemCatalogoDef("Máscara das Pessoas nas Sombras", descricao = "Concede RD Conhecimento 10 e permite entrar em uma sombra e se transportar para outra sombra visível em alcance médio.", resistencias=mapOf("Conhecimento" to 10), elemento="Conhecimento", pagina=149),
    ItemCatalogoDef("Munição Jurada", descricao = "Munição vinculada por ritual a um ser conhecido; contra esse alvo fornece bônus de ataque, aumenta a margem de ameaça e causa dano adicional de Conhecimento, mas impõe penalidade contra outros alvos.", elemento="Conhecimento", pagina=149),
    ItemCatalogoDef("Pergaminho da Pertinácia", descricao = "Concede 5 PE temporários por cena ao encarar os sigilos; usos repetidos exigem Ocultismo e podem destruir o pergaminho.", elemento="Conhecimento", pagina=149),
    ItemCatalogoDef("Arcabuz dos Moretti", categoria=2, espacos=1.0, tipoInventario="Armas", descricao = "Arma de fogo simples de uma mão que causa Energia, fornece +2 em ataques e usa um dado aleatório para determinar o dano básico. Não usa munição.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Bateria Reversa", descricao = "Absorve carga de dispositivos eletrônicos e pode transferi-la para outro dispositivo; cada uso exige Ocultismo e pode causar uma descarga perigosa se falhar.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Peitoral da Segunda Chance", descricao = "Colete que, ao reduzir o usuário a 0 PV, gasta 5 PE para reanimá-lo com 4d10 PV; cada ativação pode produzir uma descarga fatal.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Relógio de Arnaldo", descricao = "Permite gastar PE para rolar novamente dados que resultaram em 1; o custo aumenta a cada ativação no mesmo dia.", elemento="Energia", pagina=150, unico=true),
    ItemCatalogoDef("Talismã da Sorte", descricao = "Ao sofrer dano, pode gastar reação e 3 PE para tentar evitar completamente o dano; resultados extremos podem queimar o talismã ou dobrar o dano.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Teclado de Conexão Neural", descricao = "Conecta o cérebro a um computador, permitindo usar o sistema sem limitações tecnológicas ou linguísticas e recebendo bônus para hackear, mas causa dano mental por rodada.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Tela do Pesadelo", descricao = "Tela paranormal ativável que causa um efeito mental contínuo a quem a tocar, até passar no teste ou enlouquecer.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Veículo Energizado", categoria=2, espacos=1.0, tipoInventario="Itens", descricao = "Veículo que não precisa de combustível e pode assumir forma de energia por um instante para atravessar um objeto após teste de Pilotagem.", elemento="Energia", pagina=150),
    ItemCatalogoDef("Dedo Decepado", descricao = "Concede um poder paranormal que pertencia ao antigo dono; o elemento do poder define o elemento da maldição. Exige uma semana para ativar e pode impedir recuperação em interlúdios.", pagina=151),
    ItemCatalogoDef("Selos Paranormais", descricao = "Pequenos selos que armazenam um ritual. Ativá-los conjura o ritual e destrói o selo; a categoria depende do círculo do ritual.", pagina=151),
    ItemCatalogoDef("Jaqueta de Veríssimo", categoria=4, espacos=1.0, tipoInventario="Itens amaldiçoados", descricao = "Vestimenta única que concede RD Paranormal 15 e permite redirecionar para si o dano que um aliado adjacente sofreria, pagando reação e 2 PE.", resistencias=mapOf("Paranormal" to 15), pagina=151, unico=true),
)

data class MaldicaoCatalogoDef(
    val nome: String,
    val tipoAplicavel: String,
    val elemento: String,
    val descricao: String,
    val resistencias: Map<String, Int> = emptyMap(),
    val categoriaAumentoInicial: Int = 2,
    val categoriaAumentoAdicional: Int = 1,
)

/** Maldições catalogadas individualmente. Não são itens independentes: são modificações aplicadas a armas, proteções ou acessórios. */
val MALDICOES_ARMAS_CATALOGO = listOf(
    MaldicaoCatalogoDef("Antielemento","Arma","Conhecimento","A arma é letal contra criaturas de um elemento e, por 2 PE em um ataque acertado, causa +4d8 de dano. O elemento é definido conforme a regra do livro.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Ritualística","Arma","Conhecimento","Permite armazenar um ritual compatível na arma e descarregá-lo ao acertar um ataque.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Senciente","Arma","Conhecimento","Pode imbuir a arma com consciência para fazê-la flutuar e atacar ao seu lado, pagando PE para mantê-la ativa.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Empuxo","Arma","Energia","Permite arremessar a arma em alcance curto e aumenta seu dano quando usada dessa forma; aplicável a armas corpo a corpo.",elemento="Energia"),
    MaldicaoCatalogoDef("Energética","Arma","Energia","Por 2 PE, converte o dano do ataque para Energia, fornece +5 no ataque e ignora RD durante esse ataque.",elemento="Energia"),
    MaldicaoCatalogoDef("Vibrante","Arma","Energia","Concede Ataque Extra da trilha Operações Especiais ou reduz seu custo em 1 PE se já possuir a habilidade.",elemento="Energia"),
    MaldicaoCatalogoDef("Consumidora","Arma","Morte","Pode gastar 2 PE para, ao acertar, deixar o alvo imóvel por uma rodada.",elemento="Morte"),
    MaldicaoCatalogoDef("Erosiva","Arma","Morte","Causa +1d8 de Morte e pode causar dano adicional de Morte por duas rodadas ao gastar 2 PE e acertar.",elemento="Morte"),
    MaldicaoCatalogoDef("Repulsora","Arma","Morte","Concede +2 Defesa enquanto empunhada e pode fornecer +5 adicional ao fazer bloqueio por 2 PE.",elemento="Morte"),
    MaldicaoCatalogoDef("Lancinante","Arma","Sangue","Causa +1d8 de Sangue, multiplicado em acertos críticos.",elemento="Sangue"),
    MaldicaoCatalogoDef("Predadora","Arma","Sangue","Ignora camuflagem/cobertura leves, aumenta alcance de arma à distância e duplica a margem de ameaça.",elemento="Sangue"),
    MaldicaoCatalogoDef("Sanguinária","Arma","Sangue","Causa sangramento cumulativo e, em crítico, pode deixar o alvo fraco e conceder PV temporários.",elemento="Sangue"),
)

val MALDICOES_CATALOGO = listOf(
    MaldicaoCatalogoDef("Abascanta","Proteção","Conhecimento","+5 em testes de resistência contra rituais; pode refletir um ritual uma vez por cena pagando reação e PE igual ao custo.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Profética","Proteção","Conhecimento","RD Conhecimento 10 e pode repetir um teste de resistência gastando 2 PE.",resistencias=mapOf("Conhecimento" to 10),elemento="Conhecimento"),
    MaldicaoCatalogoDef("Sombria","Proteção","Conhecimento","+5 em Furtividade, ignora penalidade de carga nessa perícia e pode disfarçar a proteção como roupa comum.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Cinética","Proteção","Energia","+2 Defesa e RD 2 para proteção leve/escudo ou RD 5 para proteção pesada.",elemento="Energia"),
    MaldicaoCatalogoDef("Lépida","Proteção","Energia","+10 Atletismo, +3m deslocamento e movimento sobrenatural por 2 PE.",elemento="Energia"),
    MaldicaoCatalogoDef("Voltaica","Proteção","Energia","RD Energia 10 e pode emitir arcos que causam 2d6 Energia a adjacentes no fim dos turnos.",resistencias=mapOf("Energia" to 10),elemento="Energia"),
    MaldicaoCatalogoDef("Letárgica","Proteção","Morte","+2 Defesa e chance de ignorar dano extra de críticos e ataques furtivos.",elemento="Morte"),
    MaldicaoCatalogoDef("Repulsiva","Proteção","Morte","RD Morte 10 e pode causar 2d8 Morte a quem atacar corpo a corpo.",resistencias=mapOf("Morte" to 10),elemento="Morte"),
    MaldicaoCatalogoDef("Regenerativa","Proteção","Sangue","RD Sangue 10 e pode recuperar 1d12 PV gastando movimento e 1 PE.",resistencias=mapOf("Sangue" to 10),elemento="Sangue"),
    MaldicaoCatalogoDef("Sádica","Proteção","Sangue","Recebe bônus cumulativo em ataques e dano conforme sofreu dano desde o último turno.",elemento="Sangue"),
    MaldicaoCatalogoDef("Carisma","Acessório","Conhecimento","+1 Presença sem conceder PE adicionais.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Conjuração","Acessório","Conhecimento","Concede um ritual de 1º círculo; se já conhecer o ritual, reduz seu custo em 1 PE.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Escudo Mental","Acessório","Conhecimento","RD Mental 10.",resistencias=mapOf("Mental" to 10),elemento="Conhecimento"),
    MaldicaoCatalogoDef("Reflexão","Acessório","Conhecimento","Uma vez por rodada, pode refletir um ritual pagando PE igual ao custo.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Sagacidade","Acessório","Conhecimento","+1 Intelecto sem conceder perícias ou graus de treinamento.",elemento="Conhecimento"),
    MaldicaoCatalogoDef("Defesa","Acessório","Energia","+5 Defesa.",elemento="Energia"),
    MaldicaoCatalogoDef("Destreza","Acessório","Energia","+1 Agilidade.",elemento="Energia"),
    MaldicaoCatalogoDef("Potência","Acessório","Energia","+1 na DT de habilidades, poderes e rituais.",elemento="Energia"),
    MaldicaoCatalogoDef("Esforço Adicional","Acessório","Morte","+5 PE após um dia de uso.",elemento="Morte"),
    MaldicaoCatalogoDef("Disposição","Acessório","Sangue","+1 Vigor.",elemento="Sangue"),
    MaldicaoCatalogoDef("Pujança","Acessório","Sangue","+1 Força.",elemento="Sangue"),
    MaldicaoCatalogoDef("Vitalidade","Acessório","Sangue","+15 PV após um dia de uso.",elemento="Sangue"),
    MaldicaoCatalogoDef("Proteção Elemental","Acessório","Varia","RD 10 contra um elemento escolhido; o acessório conta como item daquele elemento.",elemento="Varia"),
)

val MALDICOES_CATALOGO_COMPLETO = MALDICOES_ARMAS_CATALOGO + MALDICOES_CATALOGO

fun patentePermiteItemAmaldicoado(patente: String): Boolean =
    patente.startsWith("Agente Especial") || patente.startsWith("Oficial de Operações") || patente.startsWith("Agente de Elite")


val CIRCULOS = listOf("1º", "2º", "3º", "4º")

fun limitePEBasePorNex(nex: Int): Int = if (nex >= 99) 20 else (nex.coerceIn(5, 95) / 5).coerceAtLeast(1)

fun circuloMaximoOcultista(nex: Int): String = when {
    nex >= 85 -> "4º"
    nex >= 55 -> "3º"
    nex >= 25 -> "2º"
    else -> "1º"
...
val ELEMENTOS_AFINIDADE = listOf("Conhecimento", "Energia", "Morte", "Sangue")

/** Elemento opressor conforme a relação circular do livro. Medo é neutro. */
fun elementoOpressor(elemento: String): String? = when (elemento) {
    "Sangue" -> "Morte"
    "Morte" -> "Energia"
...
/** Elemento que é oprimido pelo elemento informado. */
fun elementoOprimido(elemento: String): String? = when (elemento) {
    "Sangue" -> "Conhecimento"
...
fun relacaoEntreElementos(atacante: String, alvo: String): String = when {
    atacante.isBlank() || alvo.isBlank() -> "Sem elemento definido"
    atacante == "Medo" || alvo == "Medo" -> "Neutro"
    atacante == alvo -> "Mesmo elemento: +2d20 no teste de resistência"
    elementoOpressor(atacante) == alvo -> "Opressor: -2d20 no teste de resistência"
    else -> "Sem modificador pela relação elemental"
}
 ...
 */
val RITUAIS_LIVRO: List<RitualDef> = listOf(
    RitualDef(
        ...
        custoPE = 10,
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alcance para “curto” e o alvo para “um aliado à sua escolha”.",
    ),
    RitualDef(
        ...
        custoPE = 6,
        verdadeiroExtraPE = 4,
        verdadeiroDescricao = "você pode alterar ou apagar memórias de até 24 horas atrás.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o bônus de dano para +2d6.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o bônus de dano para +4d6.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda para duas modificações.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda para três modificações.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 6,
        verdadeiroExtraPE = 4,
        verdadeiroDescricao = "muda o alvo para “seres a sua escolha”.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o bônus para +2.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda o bônus para +3.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o bônus para +5 em testes de ataque.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o bônus para +5 em testes de ataque e +2 na margem de ameaça e no multiplicador de crítico.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 5,
        discenteDescricao = "muda o efeito para “fornece +10 na Defesa e resistência a balístico, corte, impacto e perfuração 5”.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 9,
        verdadeiroDescricao = "muda o efeito para “fornece +15 na Defesa e resistência a balístico, corte, impacto e perfuração 10”.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda a duração para sustentada e adiciona “Resistência: Reflexos reduz à metade”. Em vez do normal, uma vez por rodada você pode gastar uma ação de movimento para projetar uma labareda, num alvo em alcance curto. O alvo sofre 4d6 pontos de dano de Energia (Reflexos reduz à metade).",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "como discente, mas muda o dano para 8d6.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Cicatrização",
        circulo = "1º",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "aumenta a cura para 5d8+5 PV.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 9,
        verdadeiroDescricao = "muda o alcance para “curto”, o alvo para “seres escolhidos” e aumenta a cura para 7d8+7 PV.",
        requisitoVerdadeiro = "4º círculo e afinidade com Morte",
    ),
    RitualDef(
        nome = "Cinerária",
        circulo = "1º",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "além do normal, rituais conjurados dentro da névoa custam –2 PE.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "além do normal, rituais conjurados dentro da névoa causam dano maximizado.",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o alvo para aliados à sua escolha.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alvo para aliados à sua escolha e o bônus para +5.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o alcance para “curto” e o alvo para “alvos escolhidos”. Você pode entender todos os alvos afetados.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alcance para “pessoal” e o alvo para “você”. Em vez do normal, você pode falar, entender e escrever qualquer idioma humano.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda os PV temporários recebidos para 6d6.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alvo para “área: esfera com 6m de raio centrada em você” e a resistência para “Fortitude reduz à metade”. Em vez do normal, você suga energia de todos os seres vivos na área, causando 3d6 pontos de dano de Morte em cada um e recebendo PV temporários iguais ao dano total causado até o final da cena.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 6,
        discenteExtraPE = 4,
        discenteDescricao = "muda os dados de auxílio para d8. Sempre que rolar um 8 num desses dados, a entidade toma 3 pontos de sua Sanidade.",
        requisitoDiscente = "4º círculo",
        verdadeiroExtraPE = 9,
        verdadeiroDescricao = "muda os dados de auxílio para d12. Sempre que rolar um 12 num desses dados, a entidade toma 5 pontos de sua Sanidade.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "aumenta o número de laços para 6, e você pode escolher o alvo de cada laço, com um mínimo de dois laços por alvo.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "como discente, e cada laço destruído libera uma onda de choque que causa 2d6+2 pontos de dano de Energia no alvo agarrado.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 10,
        discenteExtraPE = 5,
        discenteDescricao = "muda o alvo para até cinco pessoas ou animais.",
        verdadeiroExtraPE = 10,
        verdadeiroDescricao = "muda o alvo para até dez pessoas ou animais.",
        requisitoVerdadeiro = "afinidade com Conhecimento",
    ),
    RitualDef(
        ...
        custoPE = 6,
        discenteExtraPE = 4,
        discenteDescricao = "muda o alvo para um objeto de até 10 espaços.",
        verdadeiroExtraPE = 9,
    ...
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda a duração para 1 dia.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alcance para “curto” e o alvo para “pessoas ou animais escolhidos”.",
    ),
    RitualDef(
        nome = "Decadência",
        circulo = "1º",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda a resistência para “nenhuma” e o dano para 3d8+3. Como parte da execução do ritual, você transfere as espirais para uma arma e faz um ataque corpo a corpo contra o alvo com esta arma. Se acertar, causa o dano da arma e do ritual, somados.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alcance para “pessoal” o alvo para “área: explosão com 6m de raio” e o dano para 8d8+8. As espirais afetam todos os seres na área.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Definhar",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "em vez do normal, o alvo fica exausto. Se passar na resistência, fica fatigado.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "como discente, mas muda o alvo para “até 5 seres”.",
        requisitoVerdadeiro = "3º círculo e afinidade com Morte",
    ),
    RitualDef(
        ...
        custoPE = 10,
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "afeta apenas alvos a sua escolha.",
    ),
    RitualDef(
        ...
        custoPE = 3,
        verdadeiroExtraPE = 3,
        verdadeiroDescricao = "aumenta o total de alvos para seres ou objetos somando até 100 espaços.",
    ),
    RitualDef(
        nome = "Descarnar",
        circulo = "2º",
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o dano direto para 10d8 e o dano da hemorragia para 4d8.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "além do normal, você não fica desprevenido contra perigos detectados e recebe +5 em testes de resistência contra armadilhas.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda a duração para “1 dia” e concede os mesmos benefícios de discente.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 1,
        discenteDescricao = "muda a área para “alvo: 1 objeto”. Em vez do normal, o alvo emana uma área de silêncio com 3m de raio. Se conjurar o ritual num objeto de um ser involuntário, ele tem direito a um teste de Vontade para anulá-lo.",
        verdadeiroExtraPE = 3,
        verdadeiroDescricao = "muda a duração para cena. Em vez do normal, nenhum som pode deixar a área, mas seres dentro da área podem falar, ouvir e conjurar rituais normalmente.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o alcance para “curto” e o alvo para “1 ser”. Um alvo involuntária pode anular o efeito com um teste de Vontade.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "como em Discente, mas muda o alvo para “seres escolhidos”.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Eco Espiral",
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o alvo para “até 5 seres”.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda a duração para “até 3 rodadas”, permitindo que você se concentre nas duas primeiras e descarregue na terceira.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        nome = "Eletrocussão",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o alvo para “área: linha de 30m”. Você dispara um poderoso raio que causa 6d6 pontos de dano de Energia em todos os seres e objetos livres na área.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alvo para “alvos escolhidos”. Em vez do normal, você dispara vários relâmpagos, um para cada alvo escolhido, causando 8d6 pontos de dano de Energia em cada.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Embaralhar",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o número de cópias para 5 (e o bônus na Defesa para +10).",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o número de cópias para 8 (e o bônus na Defesa para +16). Além do normal, toda vez que uma cópia é destruída, emite um clarão de luz. O ser que destruiu a cópia fica ofuscado por uma rodada.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Enfeitiçar",
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda a duração para “sustentada”. Em vez do normal, você gera uma esfera de invisibilidade. Você e todos os aliados a até 3m de você se tornam invisíveis, como no efeito normal do ritual (ainda ficam visíveis caso façam uma ação hostil). A esfera se move junto com você; qualquer coisa que saia da esfera fica visível.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda a execução para “ação padrão”, o alcance para “toque”, o alvo para “1 ser” e a duração para “sustentada”. O efeito não é dissipado caso o alvo faça um ataque ou ação hostil.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda a penalidade para –OO.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 8,
        verdadeiroDescricao = "muda a penalidade para –OO. e o alvo para “seres escolhidos”.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Ferver Sangue",
        ...
        custoPE = 6,
        verdadeiroExtraPE = 4,
        verdadeiroDescricao = "muda o alvo para “seres escolhidos”.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        nome = "Fim Inevitável",
        ...
        custoPE = 10,
        discenteExtraPE = 5,
        discenteDescricao = "muda a duração para “5 rodadas” e o efeito para que você não seja afetado.",
        requisitoDiscente = "afinidade",
        verdadeiroExtraPE = 10,
        verdadeiroDescricao = "muda a duração para “6 rodadas” e o efeito para que seres escolhidos dentro do alcance não sejam afetados.",
        requisitoVerdadeiro = "afinidade",
    ),
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o alvo para “1 ser (exceto criaturas de Sangue)”.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "como Discente, e muda a duração para “1 dia”.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 6,
        discenteExtraPE = 3,
        discenteDescricao = "além do normal, você recebe imunidade a atordoamento, fadiga, sangramento, sono e veneno.",
        verdadeiroExtraPE = 9,
        verdadeiroDescricao = "muda os bônus em testes de ataque e rolagens de dano para +10 e os PV temporários para 50.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "além do normal, seus inimigos sofrem –O em testes de ataque contra você.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "além do normal, você apura seus sentidos para perceber perigo. Você fica imune às condições surpreendido e desprevenido e recebe +10 em Defesa e Reflexos.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        nome = "Hemofagia",
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda a resistência para “nenhuma”. Como parte da execução do ritual, você faz um ataque corpo a corpo contra o alvo. Se acertar, causa o dano do ataque e do ritual, recuperando PV em quantidade igual à metade do dano total causado.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda o alcance para “pessoal”, o alvo para “você” e a duração para “cena”. Em vez do normal, a cada rodada você pode gastar uma ação padrão para tocar 1 ser e causar 4d6 pontos de dano de Sangue. Você recupera PV iguais à metade do dano causado.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        nome = "Inexistir",
        ...
        custoPE = 10,
        discenteExtraPE = 5,
        discenteDescricao = "muda o dano para 15d12+15 e o dano resistido para 3d12.",
        verdadeiroExtraPE = 10,
        verdadeiroDescricao = "muda o dano para 20d12+20 e o dano resistido para 4d12.",
        requisitoVerdadeiro = "afinidade",
    ),
    RitualDef(
        nome = "Invadir Mente",
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "se escolhar rajada mental, aumenta o dano para 10d6. Se escolher ligação telepática, em vez do normal, você cria um elo mental que permite que você gaste uma ação de movimento para ver e ouvir pelos sentidos do alvo. Um alvo involuntário pode fazer um teste de Vontade para suprimir o ritual por uma hora.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "se escolher rajada mental, aumenta o dano para 10d6 e muda o alvo para “seres escolhidos”. Se escolher ligação telepática, você pode criar um vínculo mental entre até 5 pessoas.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "a luz é cálida como a do sol. Dentro da área seus aliados recebem +O em testes de Vontade, e seus inimigos ficam ofuscados.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        nome = "Medo Tangível",
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o dano para 6d8 de Morte.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "como a versão discente, mas muda a duração para 3 rodadas. Um ser que inicie seu turno dentro da área sofre o dano novamente.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "você pode escolher seres no alcance ao conjurar o ritual; eles enxergam através do efeito.",
        requisitoDiscente = "2º círculo",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "além do normal, a nuvem fica espessa, quase sólida. Qualquer ser dentro dela tem seu deslocamento reduzido para 3m (independente de seu deslocamento normal) e sofre –2 em testes de ataque.",
        requisitoVerdadeiro = "3º círculo",
    ),
    RitualDef(
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "além do normal, sempre que o alvo usar a ação agredir, pode fazer um ataque corpo a corpo adicional contra o mesmo alvo.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o bônus de ataque e dano para +5 e o alvo passa a sofrer apenas metade do dano dos tipos balístico, corte, impacto e perfuração.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda a área para efeito: esfera de 1,5 m de diâmetro e a duração para cena. Em vez do normal, cria uma esfera de emanações espirais sibilantes com 1,5m de diâmetro que causa 4d6 pontos de dano de Morte a qualquer ser no mesmo espaço. Você pode gastar uma ação de movimento para fazer a esfera voar 9m em qualquer direção. Um ser só pode sofrer dano da esfera uma vez por rodada.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda o dano para 13d6. Seres reduzidos a 0 PV pelo dano do Paradoxo devem fazer um teste de Fortitude. Se falharem, são reduzidos a cinzas (morrem imediatamente).",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        nome = "Perturbação",
        ...
        custoPE = 1,
        discenteExtraPE = 2,
        discenteDescricao = "muda o alvo para “1 ser” e adiciona o seguinte comando: “Sofra. O alvo é acometido de dor aguda. Ele sofre 3d8 pontos de dano de Conhecimento e fica abalado por uma rodada”.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "muda o alvo para “até 5 seres” ou adiciona o seguinte comando: “Ataque. O alvo deve fazer a ação agredir contra um outro alvo a sua escolha em alcance médio, com todas as suas capacidades”.",
        requisitoVerdadeiro = "3º círculo e afinidade",
    ),
    RitualDef(
        ...
        custoPE = 6,
        verdadeiroExtraPE = 4,
        verdadeiroDescricao = "muda o dano para 4d8+16.",
    ),
    RitualDef(
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "muda o alvo para até 5 seres tocados.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 6,
        verdadeiroDescricao = "muda o alvo para até 5 seres tocados, a resistência a dano para 10 e o bônus em testes de resistência para +10.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        nome = "Purgatório",
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 2,
        discenteDescricao = "além do normal, a DT de testes de resistência contra rituais realizados na área diminui em –5.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "como discente, e o dano causado dentro da névoa por rituais é sempre mínimo.",
    ),
    RitualDef(
        nome = "Salto Fantasma",
        circulo = "3º",
        ...
        custoPE = 6,
        discenteExtraPE = 2,
        discenteDescricao = "muda a execução para reação. Em vez do normal, você salta para um espaço adjacente (1,5m), recebendo +10 na Defesa e em testes de Reflexos contra um ataque ou efeito que esteja prestes a atingi-lo.",
        verdadeiroExtraPE = 4,
        verdadeiroDescricao = "muda o alcance para longo e o alvo para você e até dois outros seres voluntários que você esteja tocando.",
    ),
    RitualDef(
        nome = "Sopro do Caos",
        circulo = "2º",
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "passa a afetar alvos Grandes.",
        verdadeiroExtraPE = 9,
        verdadeiroDescricao = "passa a afetar alvos Enormes.",
    ),
    RitualDef(
        nome = "Tecer Ilusão",
        circulo = "1º",
    ...
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "aumenta os PV temporário para 60 e a resistência para 30.",
        verdadeiroExtraPE = 7,
    ...
    RitualDef(
        ...
        custoPE = 10,
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "pode se teletransportar para qualquer local na Terra.",
    ),
    RitualDef(
        ...
        custoPE = 6,
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "aumenta o raio da área para 9m e aumenta o dano dos tentáculos para 6d6.",
    ),
    RitualDef(
        nome = "Terceiro Olho",
        circulo = "1º",
        ...
        discenteDescricao = "muda a duração para 1 dia.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "também pode enxergar objetos e seres invisíveis, que aparecem como formas translúcidas.",
    ),
    RitualDef(
        ...
        custoPE = 6,
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "aumenta o deslocamento de enchente para +12m e o dano de evaporar para 10d8.",
    ),
    RitualDef(
        ...
        custoPE = 6,
        discenteExtraPE = 3,
        discenteDescricao = "muda a área para 15 cubos com 1,5m de lado.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "também afeta todos os tipos de minerais e metais.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        ...
        circulo = "2º",
        elemento = "Sangue",
        descricao = "Você toca outro ser e transfere sua própria energia vital para ele, podendo perder até 30 pontos de vida para que o alvo recupere a mesma quantidade em PV. Você não pode ficar com menos de 1 PV por causa desse ritual.",
        execucao = "padrão",
        alcance = "toque",
        alvo = "1 ser",
        duracao = "instantânea",
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "Você pode transferir até 50 pontos de vida.",
        requisitoDiscente = "3º círculo",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "Você pode transferir até 100 pontos de vida.",
        requisitoVerdadeiro = "4º círculo",
    ),
    RitualDef(
        ...
        custoPE = 3,
        discenteExtraPE = 3,
        discenteDescricao = "em vez de uma ação de movimento, o alvo recebe uma ação padrão adicional por turno.",
        verdadeiroExtraPE = 7,
        verdadeiroDescricao = "muda o alvo para “alvos es colhidos”.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    RitualDef(
        nome = "Vidência",
    ...
    RitualDef(
        ...
        custoPE = 6,
        discenteExtraPE = 2,
        discenteDescricao = "além do normal, um alvo que falhe no teste de Reflexos fica agarrado (o enxame escala e cobre o corpo dele). O alvo pode gastar uma ação padrão e fazer um teste de Acrobacia ou Atletismo para escapar. Se você mover o enxame, o alvo fica livre.",
        verdadeiroExtraPE = 5,
        verdadeiroDescricao = "o enxame vira Enorme (cubo de 6m de lado) e ganha deslocamento de voo 18m.",
    ),
    RitualDef(
        nome = "Zerar Entropia",
        circulo = "3º",
        ...
        custoPE = 6,
        discenteExtraPE = 4,
        discenteDescricao = "muda o alvo para “1 ser”.",
        requisitoDiscente = "4º círculo",
        verdadeiroExtraPE = 11,
        verdadeiroDescricao = "muda o alvo para “seres escolhidos”.",
        requisitoVerdadeiro = "4º círculo e afinidade",
    ),
    ...
    when (r.nome) {
        "Armadura de Sangue" -> r.copy(
            discenteExtraPE = 5, verdadeiroExtraPE = 9, requisitoDiscente = "3º círculo", requisitoVerdadeiro = "4º círculo e afinidade",
        )
        "Ódio Incontrolável" -> r.copy(
            discenteExtraPE = 2, verdadeiroExtraPE = 5, requisitoVerdadeiro = "3º círculo e afinidade",
            resistenciasConcedidas = mapOf("Balístico" to 5, "Corte" to 5, "Impacto" to 5, "Perfuração" to 5),
        )
        else -> r
    }
}
