...
data class PatenteDef(
    val nome: String,
    val prestigioMin: Int,
    val limitesCategoria: Map<Int, Int>,
)

/** Patente é determinada exclusivamente por Pontos de Prestígio, nunca por NEX. */
val PATENTES_DEFINICOES = listOf(
    PatenteDef("Recruta", 0, mapOf(1 to 2)),
    PatenteDef("Operador", 20, mapOf(1 to 3, 2 to 1)),
    PatenteDef("Agente Especial", 50, mapOf(1 to 3, 2 to 2, 3 to 1)),
    PatenteDef("Oficial de Operações", 100, mapOf(1 to 3, 2 to 3, 3 to 2, 4 to 1)),
    PatenteDef("Agente de Elite", 200, mapOf(1 to 3, 2 to 3, 3 to 3, 4 to 2)),
)

val PATENTES = PATENTES_DEFINICOES.map { it.nome }

fun patentePorPrestigio(prestigio: Int): String =
    PATENTES_DEFINICOES.lastOrNull { prestigio >= it.prestigioMin }?.nome ?: "Recruta"

fun limitesCategoriaPorPrestigio(prestigio: Int): Map<Int, Int> =
    PATENTES_DEFINICOES.lastOrNull { prestigio >= it.prestigioMin }?.limitesCategoria ?: mapOf(1 to 2)
...
/** Proteções básicas. A Proteção Pesada fornece RD 2 aos quatro danos físicos
 * (balístico, corte, impacto e perfuração), não a dano paranormal em geral. */
data class ProtecaoDef(
    val nome: String,
    val categoria: Int,
    val espacos: Double,
    val defesaBonus: Int,
    val resistenciaFisico: Int,
)
