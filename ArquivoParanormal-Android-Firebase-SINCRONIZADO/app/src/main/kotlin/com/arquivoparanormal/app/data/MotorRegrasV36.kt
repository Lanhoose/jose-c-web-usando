...
fun validarPersonagemV36(p: Personagem): ValidacaoPersonagemV36 {
    ...
    if (p.classe == "Combatente") {
        val escolhas = p.periciasClasseEscolhidas.toSet()
        if (!escolhas.any { it == "Luta" || it == "Pontaria" }) erros += "Combatente: escolha Luta ou Pontaria."
        if (!escolhas.any { it == "Fortitude" || it == "Reflexos" }) erros += "Combatente: escolha Fortitude ou Reflexos."
        val extras = p.pericias.filterKeys { it !in p.periciasAutomaticas && it !in escolhas }.count { it.value.treino > 0 }
        val limiteExtras = 1 + (p.atributos["int"] ?: 1)
        if (extras > limiteExtras) erros += "Combatente: número de perícias adicionais excede 1 + Intelecto ($limiteExtras)."
    }
