...
fun calcularFicha(p: Personagem): CalculosFicha {
    ...
    if ("Tanque de Guerra" in habilidades && p.itens.any { it.equipado && it.nome == "Proteção Pesada" }) addRes("Geral", 2)
    if ("Inabalável" in habilidades || (p.trilha == "Intuitivo" && p.nex >= 65)) {
        addRes("Mental", 10)
    ...
    // RD geral é a resistência que vale para qualquer tipo de dano. Ela é
    // calculada explicitamente a partir das fontes gerais, sem duplicar as RD específicas.
    val rdGeralCalculada = run {
        var total = 0
        p.itens.filter { it.equipado }.forEach { total += it.resistencias["Geral"] ?: 0 }
        p.rituais.filter { it.ativo }.forEach { total += it.resistenciasConcedidas["Geral"] ?: 0 }
            val habilidadesRD = p.habilidades.lines().map { it.trim() }.toSet()
        if ("Mutação" in habilidadesRD) total += 2
        if ("Tanque de Guerra" in habilidadesRD && p.itens.any { it.equipado && it.nome == "Proteção Pesada" }) total += 2
        total
    }
...
fun fontesResistencia(p: Personagem, tipo: String): List<Pair<String, Int>> {
    ...
    if ("Tanque de Guerra" in habilidades && p.itens.any { it.equipado && it.nome == "Proteção Pesada" }) fontes += "Tanque de Guerra · proteção pesada" to 2
    if (("Inabalável" in habilidades || (p.trilha == "Intuitivo" && p.nex >= 65)) && tipo == "Mental") fontes += "Inabalável · trilha" to 10
    if (("Inabalável" in habilidades || (p.trilha == "Intuitivo" && p.nex >= 65)) && tipo == "Paranormal") fontes += "Inabalável · trilha" to 10
