...
data class CalculosFicha(
    ...
    val limitePE: Int,
    val limitePEParaRituais: Int,
    val resistenciasAutomaticas: Map<String, Int>,
    val limiteCarga: Int,
    val cargaMaxima: Int,
    val penalidadeDefesaCarga: Int,
    val penalidadePericiasCarga: Int,
    val penalidadeDeslocamentoCarga: Int,
    val penalidadeNaoProficiencia: Int,
    val defesaEfetiva: Int,
    val deslocamentoEfetivo: Int,
    val dtRituais: Int,
    val dtHabilidades: Int,
    val bonusDT: Int,
    val penalidadeDadosResistenciaCicatrizado: Int,
)
...
fun bonusPericiaComAtributo(p: Personagem, def: PericiaDef): Int {
    ...
    val bonusPoderes = efeitosDePoderesETrilhas(p).bonusPericia[def.nome] ?: 0
    val penalidadeCondicoes = efeitosDasCondicoes(p).penalidadePericias
    val penalidadeCarga = if (espacosUsados(p) > limiteCargaDoPersonagem(p) && def.attr in setOf("for", "agi")) -5 else 0
    return pericia.treino + pericia.outros + bonusPoderes + penalidadeCondicoes + penalidadeCarga
}

fun dadosDoAtributo(p: Personagem, def: PericiaDef): Int {
    val base = (p.atributos[def.attr] ?: 1).let { if (it <= 0) 2 else it }
    val protecaoPesadaSemProf = p.itens.any {
        (it.equipado || it.vestido) && it.nome.equals("Proteção Pesada", true) && !it.proficiente
    }
    val penalidadeCondicao = efeitosDasCondicoes(p).penalidadeDadosTestes
    val penalidadeNaoProf = if (protecaoPesadaSemProf && def.attr in setOf("for", "agi")) -2 else 0
    return (base + penalidadeCondicao + penalidadeNaoProf).coerceAtLeast(1)
}
...
fun motivoBloqueioRitual(p: Personagem, ritual: RitualDef): String? {
    ...
    if (!podeAprender) return "Você precisa ser Ocultista ou possuir a habilidade/poder Aprender Ritual para aprender rituais."
    if (p.classe != "Ocultista") {
        val limite = limiteRituaisAprenderRitual(p)
        if (limite <= 0) return "Esta ficha não possui um slot de Aprender Ritual."
        if (quantidadeRituaisAprenderRitual(p) >= limite) return "Limite de Aprender Ritual atingido: ${limite} ritual(is), igual ao Intelecto."
    }
...
fun quantidadeRituaisAprenderRitual(p: Personagem): Int =
    p.rituais.count { it.origem == "APRENDER_RITUAL" }

fun limiteRituaisAprenderRitual(p: Personagem): Int =
    if (p.poderesParanormais.any { it.nome == "Aprender Ritual" } || p.habilidades.lines().any { it.trim() == "Aprender Ritual" })
        (p.atributos["int"] ?: 1).coerceAtLeast(0)
    else 0

fun rituaisClasseOcultistaEsperados(p: Personagem): Int =
    if (p.classe != "Ocultista") 0 else 3 + if (p.nex == 99) 19 else ((p.nex - 5) / 5).coerceAtLeast(0)

fun quantidadeRituaisClasseOcultista(p: Personagem): Int =
    p.rituais.count { it.origem == "CLASSE_OCULTISTA" }

fun rituaisElegiveisParaClasseOcultista(p: Personagem, ritualParaSubstituirId: String? = null): List<RitualDef> {
    if (p.classe != "Ocultista") return emptyList()
    val max = when { p.nex >= 85 -> 4; p.nex >= 55 -> 3; p.nex >= 25 -> 2; else -> 1 }
    val esperados = rituaisClasseOcultistaEsperados(p)
    val atuais = quantidadeRituaisClasseOcultista(p)
    if (atuais >= esperados) return emptyList()
    fun num(c: String) = when (c) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
    val conhecidos = p.rituais.filterNot { it.id == ritualParaSubstituirId }.map { "${it.nome}|${it.elemento}|${it.circulo}" }.toSet()
    return RITUAIS_COMPLETOS.filter { num(it.circulo) <= max }.filterNot { "${it.nome}|${it.elemento}|${it.circulo}" in conhecidos }.distinctBy { "${it.nome}|${it.elemento}|${it.circulo}" }
}

fun rituaisElegiveisParaAprender(p: Personagem, ritualParaSubstituirId: String? = null): List<RitualDef> {
    val max = maxCirculoAprenderRitual(p.nex)
    val limite = limiteRituaisAprenderRitual(p)
    val atual = quantidadeRituaisAprenderRitual(p)
    if (limite <= 0 || atual >= limite) return emptyList()
    fun circuloNumero(c: String) = when (c) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
...
fun calcularFicha(p: Personagem): CalculosFicha {
    ...
    val sobrevivente = p.classe == "Sobrevivente"
    val estagio = p.estagioSobrevivente.coerceIn(1, 5)
    ...
    val atributoParanormal = atributoParanormalValor(p)
    val atributoPE = if (p.trilha.startsWith("Monstruoso")) atributoParanormal else pre
    val pvBase = if (sobrevivente) 8 + vig else baseDaFormula(classe?.pv ?: "16") + vig
    val pvNivel = if (sobrevivente) 2 else baseDaFormula(classe?.pvNivel ?: "3") + vig
    val peBase = if (sobrevivente) 2 + pre else baseDaFormula(classe?.pe ?: "3") + atributoPE
    val peNivel = if (sobrevivente) 1 else baseDaFormula(classe?.peNivel ?: "3")
    val sanBase = if (sobrevivente) 8 else baseDaFormula(classe?.sanidade ?: "16")
    val sanNivel = if (sobrevivente) 2 else baseDaFormula(classe?.sanNivel ?: "4")
    val survivorBonusPv = if (sobrevivente && p.trilha == "Durão" && estagio >= 2) 4 + if (estagio >= 3) 2 else 0 else 0
    val bonusTreinamentoEspecialPv = when (p.treinamentoEspecialSobrevivente) { "Combatente" -> 8; "Especialista" -> 4; else -> 0 }
    val bonusTreinamentoEspecialPe = when (p.treinamentoEspecialSobrevivente) { "Especialista" -> 1; "Ocultista" -> 2; else -> 0 }
    val bonusTreinamentoEspecialSan = when (p.treinamentoEspecialSobrevivente) { "Especialista" -> 4; "Ocultista" -> 8; else -> 0 }

    val defesaEquipamentoAuto = p.itens.filter { it.equipado || it.vestido }.sumOf { it.defesaBonus } +
        p.itens.filter { (it.equipado || it.empunhado) && it.nome.equals("Escudo", true) }.sumOf { it.defesaBonus }
    val efeitosCondicoes = efeitosDasCondicoes(p)
    val carga = espacosUsados(p)
    val limiteCargaCalculado = limiteCargaDoPersonagem(p)
    val maxCarga = cargaMaxima(p)
    val sobrecarregado = carga > limiteCargaCalculado
    val cargaAcimaDoMaximo = carga > maxCarga
    val penalidadeCargaDefesa = if (sobrecarregado) -5 else 0
    val penalidadeCargaPericias = if (sobrecarregado) -5 else 0
    val penalidadeCargaDeslocamento = if (sobrecarregado) -3 else 0
    val protecaoPesadaSemProf = p.itens.any {
        (it.equipado || it.vestido) && it.nome.equals("Proteção Pesada", true) && !it.proficiente
    }
    val penalidadeNaoProf = if (protecaoPesadaSemProf) -2 else 0
    val defesa = 10 + agi + defesaEquipamentoAuto + p.defesaEquipamento + p.defesaOutros +
        efeitosCondicoes.penalidadeDefesa + penalidadeCargaDefesa
    ...
    val limitePEBase = if (sobrevivente) 1 else limitePEBasePorNex(p.nex)
    val bonusLimitePEOrigem = if (p.origem == "Universitário") 1 else 0
    val atributoParanormal = atributoParanormalValor(p)
    val limitePE = p.overrides["limitePE"] ?: (limitePEBase + bonusLimitePEOrigem)
    val limitePEParaRituais = limitePEDeRitual(p, limitePE)
    val atributoDT = if (p.trilha.startsWith("Monstruoso")) atributoParanormal else pre
    val bonusDT = efeitosDePoderesETrilhas(p).bonusDT
    val dtRituais = dtRitualBaseComEfeitos(p, 10 + limitePEParaRituais + atributoDT)
    ...
    // Tanque de Guerra aumenta a RD física fornecida pela Proteção Pesada.
    if ("Tanque de Guerra" in habilidades && p.itens.any { (it.equipado || it.vestido) && it.nome == "Proteção Pesada" }) {
        addRes("Fisico", 2)
    }
    ...
    return CalculosFicha(
        pvMax = valor("pvMax", (if (sobrevivente) pvBase + pvNivel * (estagio - 1) + survivorBonusPv else pvBase + pvNivel * niveis + if (p.origem == "Desgarrado") p.nex / 5 else 0) + bonusTreinamentoEspecialPv),
        peMax = valor("peMax", (if (sobrevivente) peBase + peNivel * (estagio - 1) else peBase + peNivel * niveis + if (p.origem == "Universitário") 1 + (((p.nex.coerceIn(5, 99) - 5) / 10).coerceAtLeast(0)) else 0) + bonusTreinamentoEspecialPe),
        sanMax = valor("sanMax", (if (sobrevivente) sanBase + sanNivel * (estagio - 1) else sanBase + sanNivel * niveis + if (p.origem == "Vítima") p.nex / 5 else 0) + bonusTreinamentoEspecialSan),
        defesa = valor("defesa", defesa),
        esquiva = valor("esquiva", defesa + reflexos),
        ...
        limitePE = limitePE,
        limitePEParaRituais = limitePEParaRituais,
        resistenciasAutomaticas = resistAuto,
        limiteCarga = valor("limiteCarga", limiteCargaCalculado),
        cargaMaxima = maxCarga,
        penalidadeDefesaCarga = penalidadeCargaDefesa,
        penalidadePericiasCarga = if (cargaAcimaDoMaximo) -5 else penalidadeCargaPericias,
        penalidadeDeslocamentoCarga = penalidadeCargaDeslocamento,
        penalidadeNaoProficiencia = penalidadeNaoProf,
        defesaEfetiva = defesa,
        deslocamentoEfetivo = (9 + efeitosCondicoes.deslocamento + penalidadeCargaDeslocamento).coerceAtLeast(0),
        dtRituais = valor("dtRituais", dtRituais),
        dtHabilidades = valor("dtHabilidades", dtHabilidades),
        bonusDT = bonusDT,
        penalidadeDadosResistenciaCicatrizado = if (p.classe == "Sobrevivente" && p.estagioSobrevivente >= 5 && p.perigoCicatrizado.isNotBlank()) -1 else 0,
    )
}
...
fun fontesResistencia(p: Personagem, tipo: String): List<Pair<String, Int>> {
    ...
    if ("Tanque de Guerra" in habilidades && p.itens.any { (it.equipado || it.vestido) && it.nome == "Proteção Pesada" }) fontes += "Tanque de Guerra · proteção pesada" to 2
...
fun fontesRdGeral(p: Personagem): List<Pair<String, Int>> {
    ...
    if ("Tanque de Guerra" in habilidades && p.itens.any { it.equipado && it.nome == "Proteção Pesada" }) {
        fontes += "Tanque de Guerra · proteção pesada" to 2
    }
    if (p.trilha == "Tropa de Choque" && p.nex >= 99 && p.pvAtual <= p.pvMax / 2) {
...
fun sincronizarRecursosDerivados(anterior: Personagem?, novo: Personagem): Personagem {
    if (anterior == null) {
        ...
        val san = estadoSanidade(novo.copy(sanMax = calc.sanMax, sanAtual = calc.sanMax))
        return novo.copy(
            patente = patenteAtual(novo),
            possuiGrimorio = novo.possuiGrimorio || novo.habilidades.lines().any { it.trim() == "Grimório Ritualístico" },
            pvMax = calc.pvMax,
            pvAtual = calc.pvMax,
            peMax = calc.peMax,
            peAtual = calc.peMax,
    ...
    val novoSanAtual = atual(anterior.sanAtual, antigoCalc.sanMax, novoCalc.sanMax)
    val san = estadoSanidade(novo.copy(sanMax = novoCalc.sanMax, sanAtual = novoSanAtual))
    return novo.copy(
        patente = patenteAtual(novo),
        possuiGrimorio = novo.possuiGrimorio || novo.habilidades.lines().any { it.trim() == "Grimório Ritualístico" },
        ...
        sanAtual = novoSanAtual,
        perturbado = san.perturbado,
        enlouquecendo = san.enlouquecendo,
        turnosEnlouquecendo = if (san.enlouquecendo) anterior.turnosEnlouquecendo else san.turnosEnlouquecendo,
        insano = anterior.insano || san.insano,
    )
}
...
fun corrigirRecursosLegados(p: Personagem): Personagem {
    val itensCorrigidos = p.itens.map { item ->
        if (item.nome.equals("Proteção Pesada", true) && (item.resistencias["Geral"] ?: 0) > 0) {
            val rd = item.resistencias["Geral"] ?: 0
            item.copy(resistencias = (item.resistencias - "Geral") + ("Fisico" to rd))
        } else item
    }
    val base = p.copy(itens = itensCorrigidos)
    val calc = calcularFicha(base)
    val sanAtualCorrigida = if (p.sanAtual == 12 && calc.sanMax > 12) calc.sanMax else p.sanAtual.coerceAtMost(calc.sanMax)
    val san = estadoSanidade(base.copy(sanMax = calc.sanMax, sanAtual = sanAtualCorrigida))
    return base.copy(
        patente = patenteAtual(base),
        possuiGrimorio = base.possuiGrimorio || base.habilidades.lines().any { it.trim() == "Grimório Ritualístico" },
        pvMax = calc.pvMax,
        ...
        sanMax = calc.sanMax,
        sanAtual = sanAtualCorrigida,
        perturbado = san.perturbado,
        enlouquecendo = san.enlouquecendo,
        insano = p.insano || san.insano,
    )
}
