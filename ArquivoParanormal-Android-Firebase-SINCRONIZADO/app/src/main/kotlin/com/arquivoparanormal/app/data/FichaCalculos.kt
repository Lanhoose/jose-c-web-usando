...
fun possuiAprenderRitual(p: Personagem): Boolean =
    p.poderesParanormais.any { it.nome == "Aprender Ritual" } ||
        p.habilidades.lines().map { it.trim() }.contains("Aprender Ritual")

/** Quantidade de rituais que ocupam o limite global de Aprender Ritual. */
fun quantidadeRituaisPorAprenderRitual(p: Personagem): Int =
    p.rituais.count { ritual ->
        ritual.origem == "APRENDER_RITUAL" ||
            (ritual.origem == "DESCONHECIDA" && p.poderesParanormais.any { it.nome == "Aprender Ritual" && it.ritualNome == ritual.nome })
    }

/** Aprender Ritual usa slots reais limitados pelo Intelecto. Substituição libera o slot do ritual antigo. */
fun limiteRituaisAprenderRitual(p: Personagem): Int = (p.atributos["int"] ?: 0).coerceAtLeast(0)

fun possuiSlotAprenderRitual(p: Personagem, ritualParaSubstituirId: String? = null): Boolean {
    val usados = quantidadeRituaisPorAprenderRitual(p) - if (ritualParaSubstituirId != null && p.rituais.any { it.id == ritualParaSubstituirId }) 1 else 0
    return usados < limiteRituaisAprenderRitual(p)
}

/** Quantidade de rituais concedidos pela habilidade de classe Escolhido pelo Outro Lado. */
fun quantidadeRituaisOcultista(p: Personagem): Int =
    if (p.classe != "Ocultista") 0 else (3 + if (p.nex == 99) 19 else ((p.nex - 5) / 5).coerceAtLeast(0))

/** Rituais de classe do Ocultista. Eles não consomem os slots de Aprender Ritual/Intelecto. */
fun rituaisElegiveisOcultista(p: Personagem): List<RitualDef> {
    if (p.classe != "Ocultista") return emptyList()
    val max = circuloMaximoOcultista(p.nex)
    val ordem = listOf("1º", "2º", "3º", "4º")
    val conhecidos = p.rituais.map { "${it.nome}|${it.elemento}|${it.circulo}" }.toSet()
    return RITUAIS_COMPLETOS
        .filter { ordem.indexOf(it.circulo) <= ordem.indexOf(max) }
        .filterNot { "${it.nome}|${it.elemento}|${it.circulo}" in conhecidos }
        .distinctBy { "${it.nome}|${it.elemento}|${it.circulo}" }
}

fun motivoBloqueioRitual(p: Personagem, ritual: RitualDef): String? {
    if (!possuiAprenderRitual(p)) return "Este seletor exige o poder paranormal Aprender Ritual. Para Ocultista, use a seção de rituais de classe."
    ...
    if (numero > max) {
        ...
        return if (numero == 4) "O 4º círculo não é liberado por Aprender Ritual."
        else "Requer NEX $req% para Aprender Ritual de ${ritual.circulo} círculo. Seu NEX atual é ${p.nex}%."
    }
    if (!possuiSlotAprenderRitual(p)) return "Limite de Aprender Ritual atingido: seu Intelecto permite ${limiteRituaisAprenderRitual(p)} ritual(is) por esta fonte."
...
fun rituaisElegiveisParaAprender(p: Personagem, ritualParaSubstituirId: String? = null): List<RitualDef> {
    if (!possuiAprenderRitual(p)) return emptyList()
    if (!possuiSlotAprenderRitual(p, ritualParaSubstituirId)) return emptyList()
    val max = maxCirculoAprenderRitual(p.nex)
    fun circuloNumero(c: String) = when (c) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
...
fun calcularFicha(p: Personagem): CalculosFicha {
    ...
    // Tanque de Guerra aumenta a RD fornecida pela Proteção Pesada.
    // A RD base da proteção pesada é somente física (balístico, corte, impacto e perfuração).
    if ("Tanque de Guerra" in habilidades && p.itens.any { it.equipado && it.nome == "Proteção Pesada" }) {
        listOf("Balístico", "Corte", "Impacto", "Perfuração").forEach { addRes(it, 2) }
    }
    ...
    p.itens.filter { it.equipado }.forEach { item ->
        // Migração de fichas antigas: a Proteção Pesada foi armazenada como
        // "Geral" nas versões anteriores, mas a regra correta é física.
        val legadoProtecaoPesada = item.nome == "Proteção Pesada" && (item.resistencias["Geral"] ?: 0) > 0
        val geral = if (legadoProtecaoPesada) 0 else (item.resistencias["Geral"] ?: 0)
        if (geral != 0) addRes("Geral", geral)
        val fisico = (item.resistencias["Fisico"] ?: 0) + if (legadoProtecaoPesada) (item.resistencias["Geral"] ?: 0) else 0
    ...
    return CalculosFicha(
        pvMax = valor("pvMax", pvBase + pvNivel * niveis + if (p.origem == "Desgarrado") p.nex / 5 else 0),
        peMax = valor("peMax", peBase + peNivel * niveis + if (p.origem == "Universitário") 1 + (((p.nex.coerceIn(5, 99) - 5) / 10).coerceAtLeast(0)) else 0),
        ...
        limiteCarga = valor("limiteCarga", limiteDeCarga((forca + if (p.trilha == "Técnico" && p.nex >= 10) (p.atributos["int"] ?: 0) else 0))),
...
fun fontesRdGeral(p: Personagem): List<Pair<String, Int>> {
    ...
    val habilidades = p.habilidades.lines().map { it.trim() }.filter { it.isNotBlank() }.toSet()
    if ("Mutação" in habilidades) fontes += "Mutação · origem" to 2
    // Tanque de Guerra não cria RD geral; ele aumenta a RD física da proteção.
