...
data class Item(
    ...
    /** Item vestido/equipado no corpo. */
    val vestido: Boolean = false,
    /** Item atualmente empunhado; a ficha limita a dois itens empunhados. */
    val empunhado: Boolean = false,
    /** Se o personagem possui proficiência para usar este item/proteção sem penalidade. */
...
data class Arma(
    ...
    val maldicoes: List<String> = emptyList(),
    /** Marca uma arma atualmente empunhada para aplicar o limite de duas mãos. */
    val empunhada: Boolean = false,
    val proficiente: Boolean = true,
)
...
data class Ritual(
    val id: String = novoId(),
    /** Fonte que concedeu o ritual; catálogo e aquisição são coisas diferentes. */
    val origem: String = "DESCONHECIDA",
    val nome: String = "",
...
data class Personagem(
    ...
    /** Pontos de Prestígio; determina a patente independentemente do NEX. */
    val prestigio: Int = 0,
    /** Afinidade efetivamente desenvolvida após transcender novamente. */
    val afinidade: String = "",
    /** Elemento escolhido especificamente pelas trilhas Monstruoso. */
    val elementoMonstruoso: String = "",
    ...
    /** Estado mecânico da Sanidade. */
    val perturbado: Boolean = false,
    val enlouquecendo: Boolean = false,
    val turnosEnlouquecendo: Int = 0,
    /** Controle de Morrendo: três inícios de turno na mesma cena causam morte. */
    val turnosMorrendo: Int = 0,
    val morto: Boolean = false,
    val insano: Boolean = false,
    /** Histórico narrativo dos efeitos de insanidade recebidos durante cenas. */
    val historicoInsanidade: List<String> = emptyList(),
    /** Estágios próprios da classe Sobrevivente. */
    val estagioSobrevivente: Int = 1,
    val treinamentoEspecialSobrevivente: String = "",
    val perigoCicatrizado: String = "",
    val treinamentoEspecialPericias: List<String> = emptyList(),
    /** Escolhas persistentes da progressão de NEX. */
    val atributosEscolhidos: List<String> = emptyList(),
    val treinamentos35: List<String> = emptyList(),
    val treinamentos70: List<String> = emptyList(),
    val versatilidade: String = "",
    /** Estado do Grimório Ritualístico. */
    val possuiGrimorio: Boolean = false,
    val grimorioRituais: List<String> = emptyList(),
...
fun espacosUsados(p: Personagem): Double =
    p.itens.sumOf { it.espacos * it.qtd } + p.armas.sumOf { it.espacos } + if (p.possuiGrimorio || p.grimorioRituais.isNotEmpty()) 1.0 else 0.0
