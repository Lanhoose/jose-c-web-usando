...
val PODERES_TRILHA_REGRAS: List<PoderDisponivel> =
    trilha("Aniquilador", "Combatente", 26, listOf(
        Triple(10, "A Favorita", "Escolha uma arma para ser sua favorita. A categoria dessa arma é reduzida em I."),
        Triple(40, "Técnica Secreta", "A categoria da arma favorita passa a ser reduzida em II. Ao atacar com ela, pode gastar 2 PE para aplicar Amplo ou Destruidor; cada efeito adicional custa +2 PE."),
        Triple(65, "Técnica Sublime", "Adiciona Letal à Técnica Secreta (+2 na margem de ameaça, podendo escolher duas vezes para +5) e Perfurante (ignora até 5 de RD)."),
        Triple(99, "Máquina de Matar", "A categoria da arma favorita passa a ser reduzida em III; ela recebe +2 na margem de ameaça e +1 dado de dano do mesmo tipo."),
    )) +
    trilha("Comandante de Campo", "Combatente", 27, listOf(
        Triple(10, "Inspirar Confiança", "Como reação, gasta 2 PE para permitir que um aliado em alcance curto role novamente um teste recém-realizado."),
        Triple(40, "Estrategista", "Gasta uma ação padrão e 1 PE por aliado em alcance curto (máximo igual ao Intelecto). No próximo turno, cada aliado direcionado recebe uma ação de movimento adicional."),
        Triple(65, "Brecha na Guarda", "Uma vez por rodada, quando um aliado causar dano a um inimigo em alcance curto, pode gastar reação +2 PE para permitir um ataque adicional seu ou de outro aliado em alcance curto. Também aumenta para médio o alcance das duas habilidades anteriores."),
        Triple(99, "Oficial Comandante", "Gasta uma ação padrão e 5 PE para que cada aliado que possa ver em alcance médio receba uma ação padrão adicional no próximo turno."),
    )) +
    trilha("Guerreiro", "Combatente", 27, listOf(
        Triple(10, "Técnica Letal", "Recebe +2 na margem de ameaça de todos os ataques corpo a corpo."),
        Triple(40, "Revidar", "Sempre que bloquear um ataque, pode gastar reação +2 PE para fazer um ataque corpo a corpo contra o inimigo que o atacou."),
        Triple(65, "Força Opressora", "Ao acertar um ataque corpo a corpo, pode gastar 1 PE para fazer derrubar ou empurrar como ação livre. Empurrar recebe +5 para cada 10 de dano causado; ao derrubar e vencer o teste oposto, pode gastar +1 PE para atacar o alvo caído."),
        Triple(99, "Potência Máxima", "Ao usar Ataque Especial com armas corpo a corpo, todos os bônus numéricos são dobrados."),
    )) +
    trilha("Operações Especiais", "Combatente", 28, listOf(
        Triple(10, "Iniciativa Aprimorada", "Recebe +5 em Iniciativa e uma ação de movimento adicional na primeira rodada."),
        Triple(40, "Ataque Extra", "Uma vez por rodada, quando faz um ataque, pode gastar 2 PE para fazer um ataque adicional."),
        Triple(65, "Surto de Adrenalina", "Uma vez por rodada, pode gastar 5 PE para realizar uma ação padrão ou de movimento adicional."),
        Triple(99, "Sempre Alerta", "Recebe uma ação padrão adicional no início de cada cena de combate."),
    )) +
    trilha("Tropa de Choque", "Combatente", 28, listOf(
        Triple(10, "Casca Grossa", "Recebe +1 PV para cada 5% de NEX e, ao bloquear, soma Vigor à resistência a dano recebida."),
        Triple(40, "Cai Dentro", "Quando um oponente em alcance curto ataca um aliado, pode gastar reação +1 PE para fazê-lo testar Vontade (DT de Vigor). Se falhar, deve atacar você, desde que possa efetivamente fazê-lo. Um oponente que passe fica imune até o fim da cena."),
        Triple(65, "Duro de Matar", "Ao sofrer dano não paranormal, pode gastar reação +2 PE para reduzir o dano à metade. A partir de NEX 85%, também pode usar contra dano paranormal."),
        Triple(99, "Inquebrável", "Enquanto machucado, recebe +5 Defesa e RD 5. Enquanto morrendo, não fica indefeso e pode continuar realizando ações, seguindo normalmente as regras de morte."),
    )) +
    trilha("Atirador de Elite", "Especialista", 30, listOf(
        Triple(10, "Mira de Elite", "Recebe proficiência com armas de fogo que usam balas longas e soma Intelecto às rolagens de dano com essas armas."),
        Triple(40, "Disparo Letal", "Ao fazer a ação mirar, pode gastar 1 PE para receber +2 na margem de ameaça do próximo ataque até o fim do próximo turno."),
        Triple(65, "Disparo Impactante", "Ao atacar com arma de fogo, pode gastar 2 PE para, em vez de causar dano, realizar derrubar, desarmar, empurrar ou quebrar."),
        Triple(99, "Atirar para Matar", "Quando consegue um acerto crítico com arma de fogo, causa dano máximo sem rolar os dados."),
    )) +
    trilha("Infiltrador", "Especialista", 30, listOf(
        Triple(10, "Ataque Furtivo", "Uma vez por rodada, ao acertar alvo desprevenido com ataque corpo a corpo ou em alcance curto, ou alvo que esteja flanqueando, pode gastar 1 PE para causar dano extra do mesmo tipo da arma. O extra cresce para +2d6/+3d6/+4d6 nos NEX 40/65/99%."),
        Triple(40, "Gatuno", "Recebe +5 em Atletismo e Crime e pode percorrer seu deslocamento normal ao se esconder sem a penalidade correspondente."),
        Triple(65, "Assassinar", "Gasta ação de movimento +3 PE para analisar um alvo em alcance curto. Até o fim do próximo turno, o primeiro Ataque Furtivo que causar dano nele dobra os dados extras; o alvo pode ficar inconsciente ou morrendo se sofrer dano, com Fortitude (DT Agi) evitando."),
        Triple(99, "Sombra Fugaz", "Ao fazer Furtividade depois de atacar ou realizar outra ação chamativa, pode gastar 3 PE para não sofrer a penalidade do teste."),
    )) +
    trilha("Médico de Campo", "Especialista", 31, listOf(
        Triple(10, "Paramédico", "Com ação padrão +2 PE e kit de medicina, cura 2d10 PV de si ou de aliado adjacente. Nos NEX 40/65/99%, pode adicionar +1d10 por dado, pagando +1 PE por dado adicional."),
        Triple(40, "Equipe de Trauma", "Com ação padrão +2 PE e kit de medicina, remove uma condição negativa (exceto morrendo) de um aliado adjacente."),
        Triple(65, "Resgate", "Uma vez por rodada, se estiver em alcance curto de um aliado machucado ou morrendo, pode se aproximar dele com uma ação livre usando seu deslocamento normal. Sempre que curar PV ou remover condições desse aliado, você e ele recebem +5 Defesa até o início do seu próximo turno. Para você, os espaços ocupados ao carregar um personagem são reduzidos pela metade."),
        Triple(99, "Reanimação", "Uma vez por cena, gasta ação completa +10 PE para trazer de volta à vida um personagem morto na mesma cena, exceto morte por dano massivo."),
    )) +
    trilha("Negociador", "Especialista", 31, listOf(
        Triple(10, "Eloquência", "Com ação completa e 1 PE por alvo em alcance curto, faz Diplomacia, Enganação ou Intimidação contra Vontade. Se vencer, o alvo fica fascinado enquanto você se concentra; alvos hostis/em combate recebem +5 e novo teste por rodada. Quem passar fica imune por um dia."),
        Triple(40, "Discurso Motivador", "Com ação padrão +4 PE, você e aliados em alcance curto recebem bônus em testes de perícia até o fim da cena. A partir de NEX 65%, pode gastar 8 PE para aumentar o bônus."),
        Triple(65, "Eu Conheço um Cara", "Uma vez por missão, ativa sua rede de contatos para pedir um favor relevante, como trocar equipamento do grupo, conseguir descanso ou resgate. O mestre define possibilidade e alcance do favor."),
        Triple(99, "Truque de Mestre", "Gasta 5 PE para simular uma habilidade que tenha visto um aliado usar na cena. Ignora pré-requisitos, mas paga ações, PE e materiais e usa seus próprios parâmetros."),
    )) +
    trilha("Técnico", "Especialista", 31, listOf(
        Triple(10, "Inventário Otimizado", "Soma Intelecto à Força para calcular a capacidade de carga."),
        Triple(40, "Remendão", "Com ação completa +1 PE, remove a condição quebrado de equipamento adjacente até o fim da cena. Equipamentos gerais têm categoria reduzida em I para você."),
        Triple(65, "Improvisar", "Escolhe um equipamento geral e gasta ação completa +2 PE, mais 2 PE por categoria do item, para criar uma versão funcional que se torna inútil ao fim da cena."),
        Triple(99, "Preparado para Tudo", "Quando precisar de qualquer item exceto arma, pode gastar ação de movimento +3 PE por categoria para declarar que o colocou na bolsa. Depois, o item segue as regras normais de inventário."),
    )) +
    trilha("Conduíte", "Ocultista", 34, listOf(
        Triple(10, "Ampliar Ritual", "Ao lançar um ritual, pode gastar +2 PE para aumentar o alcance em um passo ou dobrar a área de efeito."),
        Triple(40, "Acelerar Ritual", "Uma vez por rodada, pode aumentar o custo de um ritual em 4 PE para conjurá-lo como ação livre."),
        Triple(65, "Anular Ritual", "Quando for alvo de ritual, pode gastar PE igual ao custo pago pelo ritual e fazer teste oposto de Ocultismo contra o conjurador. Se vencer, anula o ritual e seus efeitos."),
        Triple(99, "Canalizar o Medo", "Aprende o ritual Canalizar o Medo."),
    )) +
    trilha("Flagelador", "Ocultista", 34, listOf(
        Triple(10, "Poder do Flagelo", "Ao conjurar um ritual, pode pagar seu custo em PE com PV, à taxa de 2 PV por PE. PV gastos assim só são recuperados com descanso."),
        Triple(40, "Abraçar a Dor", "Ao sofrer dano não paranormal, pode gastar reação +2 PE para reduzir o dano à metade."),
        Triple(65, "Absorver Agonia", "Ao reduzir um ou mais inimigos a 0 PV com um ritual, recebe PE temporários iguais ao círculo do ritual."),
        Triple(99, "Medo Tangível", "Aprende o ritual Medo Tangível."),
    )) +
    trilha("Graduado", "Ocultista", 35, listOf(
        Triple(10, "Saber Ampliado", "Aprende um ritual de 1º círculo; sempre que ganha acesso a novo círculo, aprende um ritual adicional daquele círculo. Esses rituais não contam no limite de rituais."),
        Triple(40, "Grimório Ritualístico", "Cria um grimório que guarda rituais adicionais: quantidade inicial de rituais de 1º/2º círculos igual ao Intelecto e mais um quando acessa novo círculo. Não contam no limite. Para lançar um ritual do grimório, precisa empunhá-lo e gastar uma ação completa folheando para relembrá-lo."),
        Triple(65, "Rituais Eficientes", "A DT para resistir a todos os seus rituais aumenta em +5."),
        Triple(99, "Conhecendo o Medo", "Aprende o ritual Conhecendo o Medo."),
    )) +
    trilha("Intuitivo", "Ocultista", 35, listOf(
        Triple(10, "Mente Sã", "Recebe resistência paranormal +5, ou seja, +5 em testes de resistência contra efeitos paranormais."),
        Triple(40, "Presença Poderosa", "Adiciona Presença ao limite de PE por turno, mas apenas para conjurar rituais; não aumenta a DT."),
        Triple(65, "Inabalável", "Recebe resistência a dano mental e paranormal 10. Quando um efeito paranormal permite Vontade para reduzir dano à metade, um sucesso faz você não sofrer dano."),
        Triple(99, "Presença do Medo", "Aprende o ritual Presença do Medo."),
    )) +
    trilha("Lâmina Paranormal", "Ocultista", 35, listOf(
        Triple(10, "Lâmina Maldita", "Aprende Amaldiçoar Arma. Se já conhecer, pode gastar +1 PE para reduzir a execução para movimento. Ao conjurá-lo, pode usar Ocultismo em vez de Luta ou Pontaria nos ataques com a arma amaldiçoada."),
        Triple(40, "Gladiador Paranormal", "Sempre que acerta ataque corpo a corpo, recebe 2 PE temporários, até o máximo por cena igual ao seu limite de PE. Eles desaparecem ao fim da cena."),
        Triple(65, "Conjuração Marcial", "Uma vez por rodada, ao lançar ritual de ação padrão, pode gastar 2 PE para fazer um ataque corpo a corpo como ação livre."),
...
fun origemCatalogadaDoPoder(p: Personagem, nome: String): String {
    val automatico = PODERES_ORIGEM_REGRAS.firstOrNull { it.origem == p.origem && it.nome == nome }
    if (automatico != null) return "Origem: ${p.origem}"
    val classe = PODERES_CLASSE_REGRAS.firstOrNull { it.classe == p.classe && it.nome == nome }
    if (classe != null) return "Classe: ${p.classe}"
    val trilha = PODERES_TRILHA_REGRAS.firstOrNull { it.trilha == p.trilha && it.nome == nome }
    if (trilha != null) return "Trilha: ${p.trilha}"
    val geral = PODERES_GERAIS_SOBREVIVENDO.firstOrNull { it.first == nome }
    if (geral != null) return "Geral · Sobrevivendo ao Horror"
    val extra = PODERES_SOBREVIVENDO.firstOrNull { it.classe == p.classe && it.nome == nome }
    if (extra != null) return "Classe · Sobrevivendo ao Horror"
    return "Origem não catalogada · registro manual"
}
