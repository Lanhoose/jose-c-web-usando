...
fun poderesDisponiveisPara(p: Personagem): List<PoderDisponivel> {
    ...
    val extras = PODERES_SOBREVIVENDO.filter { it.classe == p.classe }.map {
        PoderDisponivel(
            ...
            classe = it.classe,
            nexMin = 15,
            descricao = "Poder adicional de Sobrevivendo ao Horror. Consulte o livro para requisitos e efeitos completos.",
            pagina = it.pagina,
            livro = it.livro,
        )
    }.filter { p.nex >= it.nexMin }
    return (origem + classe + trilha + extras).distinctBy { "${it.categoria}|${it.origem}|${it.classe}|${it.trilha}|${it.nome}" }
}
