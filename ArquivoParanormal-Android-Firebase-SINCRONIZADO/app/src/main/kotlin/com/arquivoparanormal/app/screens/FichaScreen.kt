        ...
        item {
            ...
            Painel(titulo = "Resistências a dano") {
                ...
                RESISTENCIAS.forEach { r ->
                    ...
                    if (isMestre || total > 0 || fontes.isNotEmpty()) {
                        val manualKey = "rd:$r"
                        Campo("RD $r") {
                            if (isMestre) {
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                ...
                                Chip(if (manualKey in p.overrides) "Manual · Mestre" else "Auto · Mestre", manualKey in p.overrides, { upd(p.copy(overrides = p.overrides - manualKey)) })
                            } else {
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                                    Column(Modifier.weight(1f)) {
                                        Text("$total", color = if (total > 0) Acento else TextoClaro, style = MaterialTheme.typography.titleMedium)
                                        if (fontes.isNotEmpty()) Text(fontes.joinToString(" · ") { "${it.first}: +${it.second}" }, color = TextoFraco, style = MaterialTheme.typography.labelSmall, maxLines = 3)
                                    }
            ...
            Painel(
                ...
                armaCatalogo?.let { def ->
                    Painel(
                        ...
                        var detalhesArmaCatalogoAbertos by remember(def.nome) { mutableStateOf(false) }
                        if (def.descricao.isNotBlank()) {
                            TextButton(onClick = { detalhesArmaCatalogoAbertos = true }) { Text("Ver detalhes da descrição") }
                            if (detalhesArmaCatalogoAbertos) {
                                AlertDialog(
                                    onDismissRequest = { detalhesArmaCatalogoAbertos = false },
                                    title = { Text("${def.icone} ${def.nome} — detalhes") },
                                    text = { Text(def.descricao, color = TextoClaro) },
                                    confirmButton = { TextButton(onClick = { detalhesArmaCatalogoAbertos = false }) { Text("Fechar") } },
                                )
                            }
                        }
                        Button(
                ...
                p.armas.forEach { a ->
                    ...
                    var detalhesArmaAbertos by remember(a.id) { mutableStateOf(false) }
                    ...
                    if (a.descricao.isNotBlank()) {
                        TextButton(onClick = { detalhesArmaAbertos = true }) { Text("Ver detalhes da arma") }
                        if (detalhesArmaAbertos) {
                            AlertDialog(
                                onDismissRequest = { detalhesArmaAbertos = false },
                                title = { Text("${a.icone.ifBlank { "⚔" }} ${a.nome}") },
                                text = { Text(a.descricao, color = TextoClaro) },
                                confirmButton = { TextButton(onClick = { detalhesArmaAbertos = false }) { Text("Fechar") } },
                            )
                        }
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        item {
            Painel(
                ...
                itensVisiveis.forEach { i ->
                    ...
                    var detalhesItemAbertos by remember(i.id) { mutableStateOf(false) }
                    ...
                    if (i.desc.isNotBlank()) {
                        TextButton(onClick = { detalhesItemAbertos = true }) { Text("Ver detalhes do item") }
                        if (detalhesItemAbertos) {
                            AlertDialog(
                                onDismissRequest = { detalhesItemAbertos = false },
                                title = { Text("${i.nome} — detalhes") },
                                text = { AreaTexto(i.desc, { set(i.copy(desc = it)) }, linhas = 5) },
                                confirmButton = { TextButton(onClick = { detalhesItemAbertos = false }) { Text("Fechar") } },
                            )
                        }
                    }
