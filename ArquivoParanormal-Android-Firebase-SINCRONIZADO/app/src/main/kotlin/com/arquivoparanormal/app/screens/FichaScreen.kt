                ...
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    p.condicoes.mapNotNull { nome -> CONDICOES.firstOrNull { it.nome == nome } }.forEach { c ->
                        Painel(titulo = "${c.nome}${if (c.grave) " · grave" else ""}", modifier = Modifier.fillMaxWidth()) {
                            Text(c.descricao, color = TextoClaro, style = MaterialTheme.typography.bodySmall)
                        }
                    }
                }
            }
                ...
                ritualCatalogo?.let { def ->
                    Painel(
                        ...
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            Campo("DT custo paranormal", Modifier.weight(1f)) { Text("${20 + def.custoPE}", color = Acento) }
                        }
                        Campo("Resistência / DT do ritual") {
                            Text("A resistência indicada pelo ritual usa a DT de Rituais da ficha: ${calcularFicha(p).dtRituais}.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        }
                ...
                p.rituais.forEach { r ->
                    ...
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Campo("Forma", Modifier.weight(1f)) {
                            ...
                            Selecao(r.formaSelecionada, formas, { set(r.copy(formaSelecionada = it)) })
                        }
                        Campo("Custo", Modifier.weight(1f)) { Text("$custoEfetivo PE", color = TextoClaro) }
                        Campo("DT custo paranormal", Modifier.weight(1f)) { Text("${20 + custoEfetivo}", color = Acento) }
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Campo("DT de resistência", Modifier.weight(1f)) { Text("${calcularFicha(p).dtRituais}", color = TextoClaro) }
                        Campo("Formas avançadas", Modifier.weight(2f)) {
                            Text(
                                when {
                                    r.discenteExtraPE > 0 || r.verdadeiroExtraPE > 0 -> "Discente +${r.discenteExtraPE} PE · Verdadeira +${r.verdadeiroExtraPE} PE"
                                    else -> "Não cadastradas para este ritual"
                                },
                                color = TextoFraco,
                                style = MaterialTheme.typography.bodySmall,
                            )
