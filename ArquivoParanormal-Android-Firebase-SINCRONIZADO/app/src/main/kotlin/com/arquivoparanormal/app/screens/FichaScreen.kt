...
import com.arquivoparanormal.app.data.bonusPericia
    ...
    LazyColumn(
        ...
        item {
            ...
            Painel(titulo = "Vitalidade e cálculos automáticos") {
                ...
                if (p.trilha.isNotBlank()) Text("Trilha selecionada: ${p.trilha}", color = Acento, style = MaterialTheme.typography.bodySmall)
            }
        }
        ...
        item {
            Painel(titulo = "Perícias") {
                ...
                PERICIAS.forEach { def ->
                    ...
                    val attr = p.atributos[def.attr] ?: 1
                    val total = per.treino + per.outros
                    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            Text("${def.nome} (${def.attr.uppercase()})",
                                style = MaterialTheme.typography.bodyMedium, color = TextoClaro)
                            Text("${attr}d20 ${formatBonus(total)}${if (def.nome in p.periciasAutomaticas) " · origem" else ""}",
            ...
            Painel(
                ...
                p.armas.forEach { a ->
                    ...
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(
                            onClick = {
                                val dados = p.atributos[if (a.pericia.equals("Pontaria", true)) "agi" else "for"] ?: 1
                                val bonus = ataqueDaArma(p, a)
                                val rolagens = List(dados.coerceAtLeast(1)) { Random.nextInt(1, 21) }
                                val total = rolagens.maxOrNull()!! + bonus
                                resultadoAtaque = "Rolagem: ${rolagens.joinToString(" + ")} ${formatBonus(bonus)} = $total"
                            },
                            colors = ButtonDefaults.buttonColors(containerColor = Primaria),
