            ...
            Painel(
                        ...
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            Campo("DT / custo paranormal", Modifier.weight(1f)) {
                                if (def.elemento == "Medo") {
                                    ...
                                    Text("${20 + def.custoPE}", color = Acento)
                                }
                            }
                        }
                ...
                p.rituais.forEach { r ->
                    ...
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Campo("DT / custo paranormal", Modifier.weight(1f)) {
                            if (r.elemento == "Medo") Text("Sem teste; perde SAN", color = Acento)
                            else Text("${20 + custoEfetivo}", color = Acento)
                        }
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Campo("Resistência / DT", Modifier.weight(1f)) {
                            Text(
                                if (r.resistencia.isNotBlank()) {
                                    if (r.resistencia.contains("DT 30")) r.resistencia else "${r.resistencia} · DT ${calcularFicha(p).dtRituais}"
                                } else if (r.elemento == "Medo") "Sem teste de resistência" else "DT ${calcularFicha(p).dtRituais}, quando o ritual exigir resistência",
                                color = TextoClaro,
                                style = MaterialTheme.typography.bodySmall,
                            )
                        }
