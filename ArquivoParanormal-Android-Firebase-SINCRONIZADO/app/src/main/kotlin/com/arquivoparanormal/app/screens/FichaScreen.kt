...
import com.arquivoparanormal.app.ui.ListaColapsavel
...
fun FichaScreen(repo: Repositorio, id: String, isMestre: Boolean = repo.ehMestre) {
    ...
    // Pilha de desfazer: guarda os últimos estados da ficha ANTES de cada alteração, para
    // permitir reverter um clique errado (ex.: remover um ritual sem querer). Limitada a 15
    // passos para não crescer demais em memória.
    val historicoDesfazer = remember(p.id) { androidx.compose.runtime.mutableStateListOf<Personagem>() }
    val upd: (Personagem) -> Unit = { novo ->
        val auditado = if (novo == p) novo else {
            ...
            if (eventos.isNotEmpty()) {
                historicoDesfazer.add(p)
                if (historicoDesfazer.size > 15) historicoDesfazer.removeAt(0)
            }
            eventos.fold(novo) { acc, evento -> acc.registrarAlteracao("FICHA", evento) }
        }
        repo.salvar(sincronizarRecursosDerivados(p, auditado))
    }
    val podeDesfazer = historicoDesfazer.isNotEmpty()
    val desfazer: () -> Unit = {
        if (historicoDesfazer.isNotEmpty()) {
            val anterior = historicoDesfazer.removeAt(historicoDesfazer.lastIndex)
            repo.salvar(sincronizarRecursosDerivados(p, anterior.registrarAlteracao("FICHA", "Última alteração desfeita")))
        }
    }

    val contextoCompartilhar = LocalContext.current
    var menuCompartilharAberto by remember { mutableStateOf(false) }
    var erroExportacao by remember { mutableStateOf<String?>(null) }
    val compartilharComoTexto: () -> Unit = {
        menuCompartilharAberto = false
        try {
            val intent = com.arquivoparanormal.app.data.intentCompartilharTexto(p)
            contextoCompartilhar.startActivity(Intent.createChooser(intent, "Compartilhar ficha"))
        } catch (e: Exception) {
            erroExportacao = "Não foi possível compartilhar a ficha como texto: ${e.message ?: "erro desconhecido"}"
        }
    }
    val compartilharComoPdf: () -> Unit = {
        menuCompartilharAberto = false
        try {
            val arquivo = com.arquivoparanormal.app.data.gerarPdfFicha(contextoCompartilhar, p)
            val intent = com.arquivoparanormal.app.data.intentCompartilharPdf(contextoCompartilhar, arquivo)
            contextoCompartilhar.startActivity(Intent.createChooser(intent, "Compartilhar ficha em PDF"))
        } catch (e: Exception) {
            erroExportacao = "Não foi possível gerar o PDF da ficha: ${e.message ?: "erro desconhecido"}"
        }
    }
    ...
    if (erroExportacao != null) {
        AlertDialog(
            ...
            confirmButton = { TextButton(onClick = { erroExportacao = null }) { Text("Fechar") } },
        )
    }
    ...
    LazyColumn(
            ...
            Painel(
                ...
                acao = {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        if (podeDesfazer) {
                            TextButton(onClick = desfazer) {
                                Icon(Icons.Default.Undo, contentDescription = null, tint = Acento)
                                Text("  Desfazer", color = Acento)
                            }
                        }
                        Box {
                            TextButton(onClick = { menuCompartilharAberto = true }) {
                                Icon(Icons.Default.Share, contentDescription = null, tint = Acento)
                                Text("  Compartilhar", color = Acento)
                            }
                            DropdownMenu(expanded = menuCompartilharAberto, onDismissRequest = { menuCompartilharAberto = false }) {
                                DropdownMenuItem(text = { Text("Compartilhar como texto") }, onClick = compartilharComoTexto)
                                DropdownMenuItem(text = { Text("Compartilhar como PDF") }, onClick = compartilharComoPdf)
                            }
                        }
                    }
                },
        ...
        item {
            ...
            Painel(
                ...
                ListaColapsavel("armas registradas", p.armas, buscaTexto = { "${it.nome} ${it.grupo} ${it.tipoDano}" }, vazio = "Nenhuma arma registrada.") { a ->
        ...
        item {
            Painel(
                ...
                ListaColapsavel("itens do inventário", itensVisiveis, buscaTexto = { "${it.nome} ${it.desc} ${it.tipoInventario}" }, vazio = "Mochila vazia.") { i ->
        ...
        item {
            ...
            Painel(
                ...
                ListaColapsavel("rituais aprendidos", p.rituais, buscaTexto = { "${it.nome} ${it.elemento} ${it.circulo}" }, vazio = "Nenhum ritual conhecido.") { r ->
        ...
        item {
            Painel(titulo = "Habilidades") {
                ...
                ListaColapsavel("habilidades conhecidas", nomes, buscaTexto = { it }, vazio = "Nenhuma habilidade adicionada.") { nome ->
                ...
                ListaColapsavel("poderes paranormais", p.poderesParanormais, buscaTexto = { "${it.nome} ${it.elemento} ${it.ritualNome}" }, vazio = "Nenhum poder paranormal adquirido.") { pp ->
                ...
                run {
                    val slotsClasseTranscender = quantidadePoderesDeClasseV36(p)
                    val usadosClasseTranscender = poderesDeClasseManuaisConhecidosV36(p)
                    val podeAdicionarTranscender = usadosClasseTranscender < slotsClasseTranscender
                    Text("Transcender", color = Acento, style = MaterialTheme.typography.titleSmall, modifier = Modifier.padding(top = 8.dp))
                    Text(
                        "Transcender é o poder de classe (Combatente, Especialista ou Ocultista) que permite escolher um poder paranormal, como Aprender Ritual. Ele também pode ser escolhido depois da criação, à medida que o NEX libera novas vagas de poder de classe.",
                        color = TextoFraco,
                        style = MaterialTheme.typography.bodySmall,
                    )
                    Text(
                        "Poderes de classe usados: $usadosClasseTranscender/$slotsClasseTranscender (inclui as escolhas de Transcender) · Transcender escolhido ${p.transcenderCount}x",
                        color = if (podeAdicionarTranscender) Acento else TextoFraco,
                        style = MaterialTheme.typography.labelSmall,
                    )
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button(
                            onClick = { upd(p.copy(transcenderCount = p.transcenderCount + 1)) },
                            enabled = podeAdicionarTranscender,
                        ) { Text("Escolher Transcender") }
                        if (p.transcenderCount > 0) {
                            TextButton(onClick = { upd(p.copy(transcenderCount = (p.transcenderCount - 1).coerceAtLeast(0))) }) {
                                Text("Remover uma escolha")
                            }
                        }
                    }
                    if (!podeAdicionarTranscender) {
                        Text("Sem vagas de poder de classe disponíveis no NEX atual para escolher Transcender novamente.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    }
                    if (p.transcenderCount > 0) {
                        PoderesParanormaisCriacao(p, p.transcenderCount) { novosPoderes, novosRituais ->
                            upd(p.copy(poderesParanormais = novosPoderes, rituais = novosRituais))
                        }
                    }
                }
            }
