...
import com.arquivoparanormal.app.data.limitesCategoriaPorPrestigio
import com.arquivoparanormal.app.data.quantidadeCategoriaEquipamentos
import com.arquivoparanormal.app.data.podeAdicionarEquipamento
import com.arquivoparanormal.app.data.efeitosDasCondicoes
import com.arquivoparanormal.app.data.estadoSanidade
import com.arquivoparanormal.app.data.avancarTurnoSanidade
import com.arquivoparanormal.app.data.registrarInsanidade
import com.arquivoparanormal.app.data.iniciarTurnoMorrendo
import com.arquivoparanormal.app.data.estabilizarComMedicina
import com.arquivoparanormal.app.data.espacosUsados
import com.arquivoparanormal.app.data.cargaMaxima
import com.arquivoparanormal.app.data.limiteCargaDoPersonagem
...
import com.arquivoparanormal.app.data.rituaisElegiveisParaClasseOcultista
import com.arquivoparanormal.app.data.quantidadeRituaisAprenderRitual
import com.arquivoparanormal.app.data.limiteRituaisAprenderRitual
import com.arquivoparanormal.app.data.quantidadeRituaisClasseOcultista
import com.arquivoparanormal.app.data.rituaisClasseOcultistaEsperados
...
fun FichaScreen(repo: Repositorio, id: String, isMestre: Boolean = repo.ehMestre) {
    ...
    LaunchedEffect(p.id, p.nex, p.classe, p.atributos, p.fotoArquivo, p.fotoAgenteThumb) {
        ...
        if (corrigida.pvAtual != p.pvAtual || corrigida.pvMax != p.pvMax ||
            ...
            corrigida.perturbado != p.perturbado || corrigida.enlouquecendo != p.enlouquecendo || corrigida.insano != p.insano ||
            corrigida.patente != p.patente || corrigida.possuiGrimorio != p.possuiGrimorio ||
    ...
    LazyColumn(
        ...
        item {
            Painel(titulo = "Identificação") {
                ...
                Campo("Pontos de Prestígio") {
                    Numero(p.prestigio) { upd(p.copy(prestigio = it.coerceAtLeast(0), patente = patentePorPrestigio(it.coerceAtLeast(0)))) }
                    Text("Patente automática: ${patentePorPrestigio(p.prestigio)}", color = Acento, style = MaterialTheme.typography.labelSmall)
                }
                Campo("Patente") { Text(patentePorPrestigio(p.prestigio), color = TextoClaro) }
                Campo("NEX %") {
                    if (p.classe == "Sobrevivente") Text("0% (progressão por Estágio)", color = TextoClaro)
                    else Numero(p.nex) { upd(p.copy(nex = it.coerceIn(5, 99))) }
                }
                ...
                if (p.classe != "Sobrevivente") Painel(titulo = "Progressão de NEX") {
                ...
                if (p.classe == "Sobrevivente") {
                    Painel(titulo = "Progressão por Estágios — Sobrevivente") {
                        Text("NEX permanece em 0% enquanto você for Sobrevivente. O avanço é por Estágio, do 1 ao 5.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        Selecao(p.estagioSobrevivente.toString(), (1..5).map { it.toString() }, { upd(p.copy(estagioSobrevivente = it.toInt(), nex = 0)) }, placeholder = "Estágio")
                        Text("Estágio ${p.estagioSobrevivente}: ${when (p.estagioSobrevivente) { 1 -> "Empenho"; 2 -> "Trilha"; 3 -> "Aumento de atributo"; 4 -> "Segundo poder da trilha"; else -> "Cicatrizado" }}", color = Acento)
                        if (p.estagioSobrevivente >= 2) Selecao(if (p.trilha.isBlank()) "Nenhuma" else p.trilha, listOf("Nenhuma", "Durão", "Esperto", "Esotérico"), { upd(p.copy(trilha = if (it == "Nenhuma") "" else it)) }, placeholder = "Trilha")
                        if (p.estagioSobrevivente >= 5) Selecao(p.perigoCicatrizado, ELEMENTOS_AFINIDADE, { upd(p.copy(perigoCicatrizado = it)) }, placeholder = "Perigo de Cicatrizado")
                        if (p.estagioSobrevivente >= 5 && p.treinamentoEspecialSobrevivente.isBlank()) {
                            Selecao("", listOf("Combatente", "Especialista", "Ocultista"), { classeNova -> upd(p.copy(classe = classeNova, nex = 5, treinamentoEspecialSobrevivente = classeNova)) }, placeholder = "Treinamento Especial")
                        }
                        if (p.treinamentoEspecialSobrevivente.isNotBlank()) {
                            val qtdEspecial = when (p.treinamentoEspecialSobrevivente) { "Combatente" -> 2; "Especialista" -> 6; "Ocultista" -> 4; else -> 0 }
                            Text("Treinamento Especial — perícias: ${p.treinamentoEspecialPericias.size}/$qtdEspecial", color = Acento, style = MaterialTheme.typography.labelLarge)
                            val opcoes = when (p.treinamentoEspecialSobrevivente) {
                                "Combatente" -> listOf("Fortitude", "Reflexos", "Luta", "Pontaria")
                                "Ocultista" -> listOf("Ocultismo", "Vontade") + PERICIAS.map { it.nome }.filterNot { it in listOf("Ocultismo", "Vontade") }
                                else -> PERICIAS.map { it.nome }
                            }
                            Row(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
                                opcoes.forEach { nome ->
                                    val marcada = nome in p.treinamentoEspecialPericias
                                    val grupoPermitido = when (p.treinamentoEspecialSobrevivente) {
                                        "Combatente" -> {
                                            val grupoA = setOf("Fortitude", "Reflexos")
                                            val grupoB = setOf("Luta", "Pontaria")
                                            if (nome in grupoA) p.treinamentoEspecialPericias.none { it in grupoA } else p.treinamentoEspecialPericias.none { it in grupoB }
                                        }
                                        "Ocultista" -> {
                                            val grupoBase = setOf("Ocultismo", "Vontade")
                                            if (nome in grupoBase) p.treinamentoEspecialPericias.count { it in grupoBase } < 2 else p.treinamentoEspecialPericias.count { it !in grupoBase } < 2
                                        }
                                        else -> true
                                    }
                                    if (marcada || (p.treinamentoEspecialPericias.size < qtdEspecial && grupoPermitido)) Chip(nome, marcada, {
                                        if (!marcada) {
                                            val mapa = p.pericias.toMutableMap()
                                            mapa[nome] = (mapa[nome] ?: Pericia()).copy(treino = maxOf(mapa[nome]?.treino ?: 0, 5))
                                            upd(p.copy(pericias = mapa, treinamentoEspecialPericias = (p.treinamentoEspecialPericias + nome).distinct()))
                                        }
                                    }, if (marcada) Acento else Primaria)
                                }
                            }
                        }
                    }
                ...
                Campo("Classe") {
                    if (p.classe == "Sobrevivente") Text(p.classe, color = TextoClaro)
                    else Selecao(p.classe, CLASSES.map { it.nome }, { nova -> upd(p.copy(classe = nova, trilha = "")) })
                }
                Campo("Trilha") {
        ...
        item {
            Painel(titulo = "Progressão persistente de NEX") {
                Text("As escolhas ficam gravadas na ficha. NEX apenas libera a escolha; não escolhe automaticamente por você.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                listOf(20, 50, 80, 95).filter { p.nex >= it }.forEach { marco ->
                    val chave = p.atributosEscolhidos.firstOrNull { it.startsWith("$marco:") }
                    Text("NEX $marco% — Aumento de atributo", color = Acento, style = MaterialTheme.typography.labelLarge)
                    if (chave == null) {
                        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                            ATRIBUTOS.forEach { a ->
                                Chip(a.sigla, false, {
                                    val novo = p.atributos + (a.key to ((p.atributos[a.key] ?: 1) + 1))
                                    upd(p.copy(atributos = novo, atributosEscolhidos = (p.atributosEscolhidos + "$marco:${a.key}").distinct()))
                                }, Primaria)
                            }
                        }
                    } else Text("Escolhido: ${chave.substringAfter(":").uppercase()}", color = TextoClaro)
                }
                listOf(35 to p.treinamentos35, 70 to p.treinamentos70).filter { p.nex >= it.first }.forEach { (marco, selecionados) ->
                    val qtd = when (p.classe) { "Combatente" -> 2 + (p.atributos["int"] ?: 1); "Especialista" -> 5 + (p.atributos["int"] ?: 1); "Ocultista" -> 3 + (p.atributos["int"] ?: 1); else -> 0 }
                    Text("NEX $marco% — Grau de treinamento: $qtd perícias", color = Acento, style = MaterialTheme.typography.labelLarge)
                    Text("Selecionadas: ${selecionados.size}/$qtd", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    Row(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
                        PERICIAS.forEach { def ->
                            val marcada = def.nome in selecionados
                            if (marcada || selecionados.size < qtd) {
                                Chip(def.nome, marcada, {
                                    if (!marcada && (p.pericias[def.nome]?.treino ?: 0) < 15) {
                                        val mapa = p.pericias.toMutableMap()
                                        mapa[def.nome] = (mapa[def.nome] ?: Pericia()).copy(treino = ((mapa[def.nome]?.treino ?: 0) + 5).coerceAtMost(15))
                                        val novaLista = (selecionados + def.nome).distinct()
                                        upd(if (marco == 35) p.copy(pericias = mapa, treinamentos35 = novaLista) else p.copy(pericias = mapa, treinamentos70 = novaLista))
                                    }
                                }, if (marcada) Acento else Primaria)
                            }
                        }
                ...
                if (p.nex >= 50) {
                    Text("NEX 50% — Versatilidade", color = Acento, style = MaterialTheme.typography.labelLarge)
                    if (p.versatilidade.isBlank()) {
                        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                            Chip("Poder de ${p.classe}", false, { upd(p.copy(versatilidade = "Poder de ${p.classe}")) }, Primaria)
                            Chip("1º poder de outra trilha", false, { upd(p.copy(versatilidade = "1º poder de outra trilha")) }, Primaria)
                        }
                    } else Text("Escolha registrada: ${p.versatilidade}", color = TextoClaro)
                }
                if (p.transcenderCount > 0) Text("Transcender escolhidos: ${p.transcenderCount}", color = TextoClaro)
            }
        }
        ...
        item {
            ...
            Painel(titulo = "Vitalidade e cálculos automáticos") {
                ...
                if ("Morrendo" in p.condicoes || p.pvAtual == 0) {
                    Text("Morrendo: ${p.turnosMorrendo}/3 inícios de turno nesta cena${if (p.morto) " · MORTO" else ""}", color = Perigo, style = MaterialTheme.typography.labelSmall)
                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        Button(onClick = { upd(iniciarTurnoMorrendo(p)) }, enabled = !p.morto, colors = ButtonDefaults.buttonColors(containerColor = Perigo)) { Text("Iniciar turno") }
                        Button(onClick = { upd(estabilizarComMedicina(p)) }, enabled = !p.morto, colors = ButtonDefaults.buttonColors(containerColor = Primaria)) { Text("Medicina DT 20") }
                    }
                }
                ...
                Text(
                    "Carga: ${String.format(java.util.Locale.US, "%.1f", carga)} / $limite espaços · máximo ${calc.cargaMaxima}",
                    style = MaterialTheme.typography.bodySmall,
                    color = if (carga > limite) Perigo else TextoFraco,
                )
                if (carga > limite) Text("Sobrecarga: Defesa ${calc.penalidadeDefesaCarga}, perícias afetadas ${calc.penalidadePericiasCarga}, deslocamento ${calc.penalidadeDeslocamentoCarga}m.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                if (carga > calc.cargaMaxima) Text("Carga máxima excedida: o inventário ultrapassa o dobro da capacidade.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                Text("Deslocamento efetivo: ${calc.deslocamentoEfetivo}m · Defesa efetiva: ${calc.defesaEfetiva}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                if (calc.penalidadeNaoProficiencia != 0) Text("Proteção pesada sem proficiência: –2 dados em testes baseados em Força e Agilidade.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                if (calc.penalidadeDadosResistenciaCicatrizado != 0) Text("Cicatrizado: ${calc.penalidadeDadosResistenciaCicatrizado}d20 em testes de resistência contra ${p.perigoCicatrizado}.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                ...
                if (p.nex < 50) {
                    ...
                    if (p.elementoConexao.isNotBlank()) {
                        ...
                        if (p.afinidade.isBlank()) {
                            Text("A afinidade surge automaticamente quando você recebe o próximo poder paranormal após estabelecer a conexão. Ela não é escolhida manualmente.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        }
            ...
            Painel(titulo = "Estados e condições") {
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    p.condicoes.mapNotNull { nome -> CONDICOES.firstOrNull { it.nome == nome } }.forEach { c ->
                        val efeito = com.arquivoparanormal.app.data.efeitosDaCondicao(c.nome)
                        Painel(titulo = "${c.nome}${if (c.grave) " · grave" else ""}", modifier = Modifier.fillMaxWidth()) {
                            ...
                            if (efeito.penalidadeDefesa != 0) Text("Defesa: ${efeito.penalidadeDefesa}", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.penalidadePericias != 0) Text("Perícias afetadas: ${efeito.penalidadePericias}", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.penalidadeDadosTestes != 0) Text("Dados de testes: ${efeito.penalidadeDadosTestes}d20", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.penalidadeAtaques != 0) Text("Ataques: ${efeito.penalidadeAtaques}", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.bloqueiaAcoes) Text("Bloqueia ações.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.desprevenido) Text("Fica desprevenido.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.imovel) Text("Fica imóvel.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                            if (efeito.regrasEspeciais.isNotEmpty()) Text("Regra especial: ${efeito.regrasEspeciais.joinToString(", ")}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                        }
                    }
                    val efeitosCondicoes = efeitosDasCondicoes(p)
                    Text("Efeito acumulado: Defesa ${efeitosCondicoes.penalidadeDefesa}, perícias ${efeitosCondicoes.penalidadePericias}, dados ${efeitosCondicoes.penalidadeDadosTestes}d20, ataques ${efeitosCondicoes.penalidadeAtaques}, deslocamento ${efeitosCondicoes.deslocamento}m${if (efeitosCondicoes.bloqueiaAcoes) "; ações bloqueadas" else ""}.", color = Acento, style = MaterialTheme.typography.labelSmall)
                }
            }
        }
        ...
        item {
            ...
            Painel(titulo = "Estado de Sanidade") {
                Text("${estado.atual}/${estado.maxima} SAN", color = TextoClaro, style = MaterialTheme.typography.titleMedium)
                Text(
                    when {
                        estado.insano -> "INSANO — personagem passa ao controle do Mestre."
                        estado.enlouquecendo -> "ENLOUQUECENDO — ${estado.turnosEnlouquecendo}/3 turnos."
                        estado.perturbado -> "PERTURBADO — SAN abaixo da metade."
                        else -> "Estado mental normal."
                    },
                    color = if (estado.insano || estado.enlouquecendo) Perigo else Acento,
                )
                if (estado.enlouquecendo && !estado.insano) {
                    Button(onClick = { upd(avancarTurnoSanidade(p)) }, colors = ButtonDefaults.buttonColors(containerColor = Perigo)) {
                        Text("Avançar turno de Enlouquecendo")
                    }
                }
                Campo("Registrar efeito de insanidade") {
                    var texto by remember(p.historicoInsanidade.size) { mutableStateOf("") }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        CampoTexto(texto, Modifier.weight(1f), aoMudar = { texto = it })
                        Button(onClick = { if (texto.isNotBlank()) { upd(registrarInsanidade(p, texto)); texto = "" } }, colors = ButtonDefaults.buttonColors(containerColor = Primaria)) { Text("Salvar") }
                    }
                }
                p.historicoInsanidade.takeLast(8).forEach { Text("• $it", color = TextoFraco, style = MaterialTheme.typography.bodySmall) }
            }
        }
        ...
        item {
            val temGrimorio = p.habilidades.lines().any { it.trim() == "Grimório Ritualístico" } || p.poderesAutomaticos.contains("Grimório Ritualístico")
            if (temGrimorio) {
                val slots = (p.atributos["int"] ?: 1).coerceAtLeast(0) + (if (p.nex >= 55) 1 else 0) + (if (p.nex >= 85) 1 else 0)
                val selecionados = p.grimorioRituais.distinct()
                var ritualGrimorioSelecionado by remember { mutableStateOf("") }
                val disponiveis = RITUAIS_COMPLETOS.filter { def ->
                    val n = when (def.circulo) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
                    n <= if (p.nex >= 85) 4 else if (p.nex >= 55) 3 else 2
                }.filterNot { "${it.nome}|${it.elemento}|${it.circulo}" in selecionados }.distinctBy { "${it.nome}|${it.elemento}|${it.circulo}" }
                Painel(titulo = "Grimório Ritualístico") {
                    Text("${selecionados.size}/$slots rituais armazenados. O grimório ocupa 1 espaço e precisa ser empunhado; folhear exige uma ação completa antes de conjurar o ritual relembrado.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                    Chip(if (p.grimorioEmpunhado) "Grimório empunhado" else "Grimório guardado", p.grimorioEmpunhado, {
                        val outros = p.itens.count { it.empunhado } + p.armas.count { it.empunhada }
                        if (p.grimorioEmpunhado || outros < 2) upd(p.copy(grimorioEmpunhado = !p.grimorioEmpunhado))
                    }, if (p.grimorioEmpunhado) Acento else Primaria)
                    if (selecionados.size < slots) {
                        Selecao(ritualGrimorioSelecionado, disponiveis.map { "${it.nome} — ${it.circulo} · ${it.elemento}" }, { ritualGrimorioSelecionado = it }, placeholder = "Adicionar ritual ao grimório")
                        Button(onClick = {
                            val def = disponiveis.firstOrNull { "${it.nome} — ${it.circulo} · ${it.elemento}" == ritualGrimorioSelecionado }
                            if (def != null) {
                                upd(p.copy(grimorioRituais = (selecionados + "${def.nome}|${def.elemento}|${def.circulo}").distinct()))
                                ritualGrimorioSelecionado = ""
                            }
                        }, enabled = ritualGrimorioSelecionado.isNotBlank(), colors = ButtonDefaults.buttonColors(containerColor = Primaria)) { Text("Adicionar ao grimório") }
                    }
                    selecionados.forEach { chave ->
                        val partes = chave.split("|")
                        val nome = partes.getOrNull(0) ?: chave
                        val elem = partes.getOrNull(1) ?: ""
                        val circ = partes.getOrNull(2) ?: ""
                        val relemb = p.ritualGrimorioRelembradoId == chave
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                            Column(Modifier.weight(1f)) {
                                Text("$nome — $circ · $elem", color = TextoClaro)
                                if (relemb) Text("Relembrado após folhear", color = Acento, style = MaterialTheme.typography.labelSmall)
                            }
                            TextButton(onClick = { if (p.grimorioEmpunhado) upd(p.copy(ritualGrimorioRelembradoId = if (relemb) null else chave)) }) { Text(if (relemb) "Esquecer" else "Folhear") }
                            TextButton(onClick = { upd(p.copy(grimorioRituais = selecionados - chave, ritualGrimorioRelembradoId = if (relemb) null else p.ritualGrimorioRelembradoId)) }) { Text("Remover") }
                        }
                    }
                }
            }
        ...
        item {
            ...
            Painel(titulo = "Resistências a dano") {
                ...
                Painel(titulo = "Calcular dano após resistência", modifier = Modifier.fillMaxWidth()) {
                    ...
                    if (danoFinal * 2 >= calc.pvMax && danoFinal > 0 && p.pvAtual > 0) {
                        Text("DANO MASSIVO: Fortitude DT ${15 + 2 * (danoFinal / 10)} se o alvo sobreviver ao dano.", color = Perigo, style = MaterialTheme.typography.labelSmall)
                    }
        ...
        item {
            ...
            Painel(
                ...
                p.armas.forEach { a ->
                    ...
                    Campo("Categoria") {
                        Selecao(
                            ...
                            { label ->
                                ...
                                val semAtual = p.copy(itens = p.itens, armas = p.armas.filterNot { it.id == a.id })
                                if (podeAdicionarEquipamento(semAtual, c.cat, isMestre)) set(a.copy(categoria = c.cat, espacos = c.espacos))
                            },
                        )
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        Chip(if (a.empunhada) "Empunhada" else "Guardada", a.empunhada, {
                            val outros = p.itens.count { it.empunhado } + p.armas.count { it.empunhada && it.id != a.id } + if (p.grimorioEmpunhado) 1 else 0
                            if (!a.empunhada && outros < 2) set(a.copy(empunhada = true)) else if (a.empunhada) set(a.copy(empunhada = false))
                        })
                        Chip(if (a.proficiente) "Proficiente" else "Não proficiente", a.proficiente, { set(a.copy(proficiente = !a.proficiente)) }, if (a.proficiente) Primaria else Perigo)
                    }
                    Button(
        ...
        item {
            Painel(
                ...
                val limitesPatente = limitesCategoriaPorPrestigio(p.prestigio)
                Text("Limites por patente (${patentePorPrestigio(p.prestigio)}): " +
                    (1..4).joinToString(" · ") { cat -> "Cat. $cat ${quantidadeCategoriaEquipamentos(p, cat)}/${limitesPatente[cat] ?: 0}" },
                    color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                if (cargaExcedida) {
                ...
                protecaoDef?.let { def ->
                    Button(onClick = {
                        val resist = if (def.resistenciaFisico > 0) mapOf("Fisico" to def.resistenciaFisico) else emptyMap()
                        upd(p.copy(itens = p.itens + Item(nome = def.nome, categoria = def.categoria, espacos = def.espacos, tipoInventario = "Proteções", equipado = true, vestido = true, proficiente = def.nome != "Proteção Pesada" || p.habilidades.lines().any { it.trim() == "Proteção Pesada" }, defesaBonus = def.defesaBonus, resistencias = resist)))
                        protecaoSelecionada = ""
                    }, enabled = podeAdicionarEquipamento(p, def.categoria, isMestre), colors = ButtonDefaults.buttonColors(containerColor = Primaria)) {
                ...
                Selecao(
                    ...
                    placeholder = if (p.prestigio >= 50 || isMestre) "Selecione um item catalogado" else "Requer Agente Especial ou patente superior",
                )
                ...
                itemAmaldicoadoDef?.let { def ->
                    ...
                    Button(
                        enabled = (!jaExiste || !def.unico) && podeAdicionarEquipamento(p, def.categoria, isMestre),
                        onClick = {
                            val novo = Item(
                                nome = def.nome,
                                categoria = def.categoria,
                ...
                if (p.prestigio < 50 && !isMestre) {
                    Text("Itens amaldiçoados deste catálogo exigem patente mínima de Agente Especial.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                }

                // Maldições são modificações, não itens independentes. O seletor abaixo só
                // oferece maldições catalogadas e aplica os efeitos mecânicos ao item/arma escolhido.
                ...
                val alvosItens = p.itens.filter { it.tipoInventario in listOf("Proteções", "Itens", "Itens amaldiçoados", "Outros") }
                val alvosArmas = p.armas
                Text("Aplicar maldição catalogada", style = MaterialTheme.typography.labelSmall, color = TextoFraco)
                Selecao(alvoMaldicao, listOf("Item: ${alvosItens.firstOrNull()?.nome ?: "nenhum"}") + alvosItens.drop(1).map { "Item: ${it.nome}" } + alvosArmas.map { "Arma: ${it.nome}" }, { alvoMaldicao = it }, placeholder = "Escolha o item/arma")
                ...
                val maldicoesElegiveis = MALDICOES_CATALOGO_COMPLETO.filter { def ->
                    (tipoAlvoMaldicao == "Arma" && def.tipoAplicavel == "Arma") ||
                        (tipoAlvoMaldicao == "Item" && def.tipoAplicavel in listOf("Proteção", "Acessório"))
                }
                ...
                maldicaoDef?.let { def ->
                    val alvoItem = alvosItens.firstOrNull { it.nome == nomeAlvoMaldicao }
                    val alvoArma = alvosArmas.firstOrNull { it.nome == nomeAlvoMaldicao }
                    val maldicoesExistentes = if (alvoItem != null) alvoItem.maldicoes else alvoArma?.maldicoes ?: emptyList()
                    ...
                    Button(
                        enabled = (alvoItem != null || alvoArma != null) && def.nome !in maldicoesExistentes && !conflitoElemento && (def.nome != "Proteção Elemental" || elementoMaldicao.isNotBlank()),
                        onClick = {
                            if (alvoItem != null) {
                                val rd = when {
                                    def.nome == "Proteção Elemental" && elementoMaldicao.isNotBlank() -> mapOf(elementoMaldicao to 10)
                                    def.nome == "Cinética" -> mapOf("Fisico" to if (alvoItem.tipoInventario == "Proteções" && alvoItem.espacos >= 5.0) 5 else 2)
                                    else -> def.resistencias
                                }
                                val atualizado = alvoItem.copy(maldicoes = alvoItem.maldicoes + def.nome, resistencias = alvoItem.resistencias + rd)
                                upd(p.copy(itens = p.itens.map { if (it.id == alvoItem.id) atualizado else it }))
                            } else if (alvoArma != null) {
                                val atualizado = alvoArma.copy(maldicoes = alvoArma.maldicoes + def.nome)
                ...
                itensVisiveis.forEach { i ->
                    ...
                    Campo("Categoria") {
                        Selecao(
                            ...
                            { label ->
                                val c = CATEGORIAS_ITEM.first { it.label == label }
                                val semAtual = p.copy(itens = p.itens.filterNot { it.id == i.id }, armas = p.armas.filterNot { it.id == i.id })
                                if (podeAdicionarEquipamento(semAtual, c.cat, isMestre)) set(i.copy(categoria = c.cat, espacos = c.espacos))
                            },
                        )
                    }
                    Campo("Descrição") { AreaTexto(i.desc, { set(i.copy(desc = it)) }, linhas = 2) }
                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        Chip(if (i.equipado) "Equipado" else "Guardado", i.equipado, { set(i.copy(equipado = !i.equipado, vestido = !i.equipado || i.vestido)) })
                        Chip(if (i.empunhado) "Empunhado" else "Não empunhado", i.empunhado, {
                            val outros = p.itens.count { it.empunhado && it.id != i.id } + p.armas.count { it.empunhada } + if (p.grimorioEmpunhado) 1 else 0
                            if (i.empunhado || outros < 2) set(i.copy(empunhado = !i.empunhado))
                        })
                        Chip(if (i.proficiente) "Proficiente" else "Não proficiente", i.proficiente, { set(i.copy(proficiente = !i.proficiente)) }, if (i.proficiente) Primaria else Perigo)
        ...
        item {
            ...
            var fonteRitual by remember(p.classe, p.nex, p.rituais.size, p.poderesParanormais.size) { mutableStateOf(if (p.classe == "Ocultista" && quantidadeRituaisClasseOcultista(p) < rituaisClasseOcultistaEsperados(p)) "Classe: Escolhido pelo Outro Lado" else "Aprender Ritual") }
            val possuiAprenderRitual = p.poderesParanormais.any { it.nome == "Aprender Ritual" } || p.habilidades.lines().map { it.trim() }.contains("Aprender Ritual")
            val rituaisPermitidosAprender = if (possuiAprenderRitual) rituaisElegiveisParaAprender(p) else emptyList()
            val rituaisPermitidosClasse = if (p.classe == "Ocultista") rituaisElegiveisParaClasseOcultista(p) else emptyList()
            val rituaisPermitidos = if (fonteRitual.startsWith("Classe")) rituaisPermitidosClasse else rituaisPermitidosAprender
            val podeAprenderRitual = rituaisPermitidos.isNotEmpty()
            ...
            Painel(
                ...
                acao = {
                    Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                        IconButton(onClick = {
                            upd(p.copy(rituais = p.rituais + Ritual(origem = "MESTRE_MANUAL", nome = "Novo ritual")))
                        }) {
                ...
                Text("Aquisição regulamentada — a fonte é registrada no ritual.", style = MaterialTheme.typography.labelSmall, color = TextoFraco)
                if (p.classe == "Ocultista" && (rituaisPermitidosClasse.isNotEmpty() || possuiAprenderRitual)) {
                    Selecao(
                        fonteRitual,
                        buildList {
                            if (rituaisPermitidosClasse.isNotEmpty()) add("Classe: Escolhido pelo Outro Lado")
                            if (possuiAprenderRitual) add("Aprender Ritual")
                        },
                        { fonteRitual = it; ritualLivroSelecionado = ""; ritualElementoEscolhido = "" },
                        placeholder = "Fonte da aquisição",
                    )
                }
                Text(
                    when {
                        fonteRitual.startsWith("Classe") -> "Classe Ocultista · ${quantidadeRituaisClasseOcultista(p)}/${rituaisClasseOcultistaEsperados(p)} rituais de Escolhido pelo Outro Lado · círculo máximo ${circuloMaximoOcultista(p.nex)}."
                        possuiAprenderRitual -> "Aprender Ritual · ${quantidadeRituaisAprenderRitual(p)}/${limiteRituaisAprenderRitual(p)} rituais (limite = Intelecto) · até ${maxCirculoAprenderRitual(p.nex)}º círculo."
                        else -> "Nenhuma fonte de aquisição disponível."
                    },
                                ...
                                upd(
                                    p.copy(
                                        rituais = p.rituais + Ritual(
                                            origem = if (p.classe == "Ocultista" && rituaisPermitidosClasse.any { it.nome == def.nome && it.circulo == def.circulo && it.elemento == def.elemento }) "CLASSE_OCULTISTA" else "APRENDER_RITUAL",
                                            nome = def.nome,
                                            circulo = def.circulo,
                ...
                p.rituais.forEach { r ->
                    ...
                    Text("Origem: ${r.origem.replace('_', ' ')}", color = Acento, style = MaterialTheme.typography.labelSmall)
