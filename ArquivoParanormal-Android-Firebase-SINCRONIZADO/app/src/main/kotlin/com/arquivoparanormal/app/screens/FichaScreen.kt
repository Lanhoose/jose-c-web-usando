...
import com.arquivoparanormal.app.data.possuiAprenderRitual
import com.arquivoparanormal.app.data.quantidadeRituaisPorAprenderRitual
import com.arquivoparanormal.app.data.limiteRituaisAprenderRitual
import com.arquivoparanormal.app.data.quantidadeRituaisOcultista
import com.arquivoparanormal.app.data.rituaisElegiveisOcultista
...
import com.arquivoparanormal.app.data.quantidadePoderesDeClasse
import com.arquivoparanormal.app.data.poderesManuaisSelecionados
    ...
    LazyColumn(
        ...
        item {
            Painel(
                ...
                protecaoDef?.let { def ->
                    Button(onClick = {
                        val resist = if (def.resistenciaFisica > 0) mapOf("Fisico" to def.resistenciaFisica) else emptyMap()
        ...
        item {
            var ritualClasseSelecionado by remember { mutableStateOf("") }
            var ritualAprenderSelecionado by remember { mutableStateOf("") }
            var ritualElementoEscolhido by remember { mutableStateOf("") }
            val temAprenderRitual = possuiAprenderRitual(p)
            val rituaisAprender = rituaisElegiveisParaAprender(p)
            val totalAprender = limiteRituaisAprenderRitual(p)
            val usadosAprender = quantidadeRituaisPorAprenderRitual(p)
            val totalOcultista = quantidadeRituaisOcultista(p)
            val usadosOcultista = p.rituais.count { it.origem == "CLASSE_OCULTISTA" || (it.origem == "DESCONHECIDA" && it.origem != "APRENDER_RITUAL" && it.origem != "MESTRE_MANUAL") }
            val rituaisClasse = if (p.classe == "Ocultista" && usadosOcultista < totalOcultista) rituaisElegiveisOcultista(p) else emptyList()

            val ritualClasseDef = rituaisClasse.firstOrNull { "${it.simbolo} ${it.nome} — ${it.circulo} · ${it.elemento}" == ritualClasseSelecionado }
            val ritualAprenderDef = rituaisAprender.firstOrNull { "${it.simbolo} ${it.nome} — ${it.circulo} · ${it.elemento}" == ritualAprenderSelecionado }
            val ritualCatalogoBase = ritualClasseDef ?: ritualAprenderDef
            val origemSelecao = if (ritualClasseDef != null) "CLASSE_OCULTISTA" else "APRENDER_RITUAL"
            ...
            Painel(
                ...
                acao = {
                    Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                        IconButton(onClick = { upd(p.copy(rituais = p.rituais + Ritual(nome = "Novo ritual", origem = "MESTRE_MANUAL"))) }) {
                            Icon(Icons.Default.Add, contentDescription = "Novo ritual", tint = Primaria)
                        }
                    }
                },
            ) {
                Text("As fontes de rituais ficam separadas: rituais de classe do Ocultista não usam Intelecto; Aprender Ritual usa slots limitados pelo Intelecto.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)

                if (p.classe == "Ocultista") {
                    Text("Escolhido pelo Outro Lado · Classe", color = Acento, style = MaterialTheme.typography.titleSmall)
                    Text("Slots de classe: $usadosOcultista/$totalOcultista · círculo máximo ${com.arquivoparanormal.app.data.circuloMaximoOcultista(p.nex)}. Não consome o limite de Intelecto de Aprender Ritual.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    if (rituaisClasse.isEmpty()) {
                        Text(if (usadosOcultista >= totalOcultista) "Todos os slots de rituais de classe já foram preenchidos." else "Nenhum ritual de classe novo disponível neste NEX.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                    } else {
                        Selecao(
                            valor = ritualClasseSelecionado,
                            opcoes = rituaisClasse.map { "${it.simbolo} ${it.nome} — ${it.circulo} · ${it.elemento}" },
                            aoMudar = { ritualClasseSelecionado = it; ritualAprenderSelecionado = ""; ritualElementoEscolhido = "" },
                            placeholder = "Escolha um ritual de classe",
                        )
                    }
                }

                if (temAprenderRitual) {
                    Text("Aprender Ritual · Poder paranormal", color = Acento, style = MaterialTheme.typography.titleSmall)
                    Text("Slots por Intelecto: $usadosAprender/$totalAprender · NEX ${p.nex}% permite até o ${maxCirculoAprenderRitual(p.nex)}º círculo. Substituição libera o slot do ritual antigo.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    if (usadosAprender >= totalAprender) {
                        Text("Limite de Aprender Ritual atingido. Aumente Intelecto ou substitua um ritual aprendido por esta fonte.", color = Perigo, style = MaterialTheme.typography.bodySmall)
                    } else {
                        Selecao(
                            valor = ritualAprenderSelecionado,
                            opcoes = rituaisAprender.map { "${it.simbolo} ${it.nome} — ${it.circulo} · ${it.elemento}" },
                            aoMudar = { ritualAprenderSelecionado = it; ritualClasseSelecionado = ""; ritualElementoEscolhido = "" },
                            placeholder = "Escolha um ritual para Aprender Ritual",
                        )
                    }
                } else if (p.classe != "Ocultista") {
                    Text("Esta ficha não possui Aprender Ritual. Rituais de classe só aparecem para Ocultista.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                }
                ...
                val rituaisBloqueados = (if (temAprenderRitual) {
                    RITUAIS_COMPLETOS.filter { ritual -> motivoBloqueioRitual(p, ritual) != null }
                } else {
                    emptyList()
                }).distinctBy { ritual ->
                    if (ritual.nome == "Amaldiçoar Arma") "${ritual.nome}|${ritual.circulo}"
                    else "${ritual.nome}|${ritual.elemento}|${ritual.circulo}"
                }
                ...
                ritualCatalogo?.let { def ->
                    var detalhesCatalogoAbertos by remember(ritualClasseSelecionado, ritualAprenderSelecionado) { mutableStateOf(false) }
                    Painel(
                        ...
                        Text("Origem da aquisição: ${if (origemSelecao == "CLASSE_OCULTISTA") "Escolhido pelo Outro Lado · classe" else "Aprender Ritual · limite de Intelecto"}", color = Acento, style = MaterialTheme.typography.labelSmall)
                        Text("Fonte do ritual: ${def.fonte}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                        Row(
                            Modifier.fillMaxWidth(),
                            horizontalArrangement = Arrangement.spacedBy(8.dp),
                        ) {
                        ...
                        Button(
                            onClick = {
                                ...
                                ritualClasseSelecionado = ""; ritualAprenderSelecionado = ""
                            },
                            colors = ButtonDefaults.buttonColors(containerColor = Primaria),
                        ) {
                            Icon(Icons.Default.Add, contentDescription = null)
                ...
                p.rituais.forEach { r ->
                    ...
                    Text("Aquisição: ${when (r.origem) {
                        "CLASSE_OCULTISTA" -> "Escolhido pelo Outro Lado · classe"
                        "APRENDER_RITUAL" -> "Aprender Ritual · slot de Intelecto"
                        "TRILHA" -> "Trilha"
                        "SOBREVIVENTE_INICIADO" -> "Sobrevivente · Iniciado"
                        "MESTRE_MANUAL" -> "Cadastro manual do Mestre"
                        else -> "Origem não registrada (ficha antiga)"
                    }}", color = Acento, style = MaterialTheme.typography.labelSmall)
                    if (r.origem == "DESCONHECIDA") Text("Esta ficha antiga não registrava a origem do ritual; isso não o transforma automaticamente em ritual de Aprender Ritual.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
        ...
        item {
            Painel(titulo = "Perfil do agente") {
                ...
                val nomesJaEscolhidos = poderesManuaisSelecionados(p).toSet()
                val poderesPermitidosBase = com.arquivoparanormal.app.data.poderesDisponiveisPara(p)
                    .filter { !it.automatico }
                    .filter { it.nome != "Transcender" }
                val poderesPermitidos = poderesPermitidosBase.filterNot { it.nome in nomesJaEscolhidos }
                val limiteClasse = quantidadePoderesDeClasse(p)
                val poderesClasseEscolhidos = catalogoCompletoDePoderes().count { it.categoria == "Classe" && it.nome in nomesJaEscolhidos }
                val slotsClasseRestantes = (limiteClasse - poderesClasseEscolhidos).coerceAtLeast(0)
                ...
                Text("A ficha não permite escolher novamente um poder já registrado nas etapas de criação/progressão. Poderes de classe também respeitam os slots do NEX; poderes automáticos e Transcender ficam separados.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                Text("Slots de poderes de classe: $poderesClasseEscolhidos/$limiteClasse · restantes: $slotsClasseRestantes", color = Acento, style = MaterialTheme.typography.labelSmall)
                Campo("Escolher poder") {
                    ...
                    poderCatalogado?.let { def ->
                        ...
                        val podeAdicionarPoder = def.nome !in nomesJaEscolhidos && (def.categoria != "Classe" || slotsClasseRestantes > 0)
                        TextButton(
                            enabled = podeAdicionarPoder,
                            onClick = {
                                if (!podeAdicionarPoder) return@TextButton
                                val linha = def.nome
                                val novo = if (p.habilidades.isBlank()) linha else p.habilidades + "\n" + linha
                                upd(p.copy(habilidades = novo))
                                poderSelecionado = ""
                            },
                        ) { Text(if (def.categoria == "Classe" && slotsClasseRestantes <= 0) "Limite de poderes de classe atingido" else "Adicionar poder") }
                    }
                }
