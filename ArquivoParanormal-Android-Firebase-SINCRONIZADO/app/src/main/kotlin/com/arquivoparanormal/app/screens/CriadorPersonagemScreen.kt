...
import com.arquivoparanormal.app.data.PODERES_TRILHA_REGRAS
...
fun CriadorPersonagemScreen(
    ...
    fun patch(novo: Personagem) {
        ...
        // Mantém os estados locais das etapas sincronizados com a ficha.
        poderes = normalizado.habilidades.lines().filter { it.isNotBlank() }.toSet()
        pericias = normalizado.pericias.filterValues { it.treino > 0 }.keys.toSet()
        armas = normalizado.armas.map { it.nome }.toSet()
...
private fun OrigemCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    Painel(titulo="Etapa 3 — Origem") {
        Selecao(p.origem, ORIGENS_COMPLETAS.map{it.nome}, { novaOrigem ->
            ...
            val novasClasse = periciasIniciaisDaClasse(p.classe, novas.toSet(), p.periciasClasseAutomaticas)
            p.periciasClasseAutomaticas.forEach { nome ->
                if (nome.isNotBlank() && nome !in novas && nome !in novasClasse) mapa.remove(nome)
            }
            novasClasse.filter { it.isNotBlank() }.forEach { nome -> mapa[nome] = (mapa[nome] ?: Pericia()).copy(treino = 5) }
            patch(p.copy(origem=novaOrigem, pericias=mapa, periciasAutomaticas=novas, periciasClasseAutomaticas=novasClasse))
...
private fun ClasseCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    val classeAtual = CLASSES.firstOrNull { it.nome == p.classe }
    val origemPericias = p.periciasAutomaticas.toSet()

    LaunchedEffect(p.classe, p.origem) {
        if (p.classe == "Combatente" || p.classe == "Ocultista") {
            val atuais = p.periciasClasseAutomaticas
            val normalizadas = periciasIniciaisDaClasse(p.classe, origemPericias, atuais)
            if (atuais != normalizadas) {
                val mapa = p.pericias.toMutableMap()
                atuais.filter { it.isNotBlank() && it !in origemPericias && it !in normalizadas }.forEach { mapa.remove(it) }
                normalizadas.filter { it.isNotBlank() }.forEach { mapa[it] = (mapa[it] ?: Pericia()).copy(treino = 5) }
                patch(p.copy(pericias = mapa, periciasClasseAutomaticas = normalizadas))
            }
        }
    }

    fun aplicarClasse(novaClasse: String) {
        val antigasClasse = p.periciasClasseAutomaticas.toSet()
        val mapa = p.pericias.toMutableMap()
        antigasClasse.forEach { nome ->
            if (nome !in p.periciasAutomaticas) mapa.remove(nome)
        }
        val novas = periciasIniciaisDaClasse(novaClasse, origemPericias, emptyList())
        novas.forEach { nome -> mapa[nome] = (mapa[nome] ?: Pericia()).copy(treino = 5) }
        patch(p.copy(classe = novaClasse, trilha = "", pericias = mapa, periciasClasseAutomaticas = novas))
    }

    fun definirClassePericia(index: Int, nome: String) {
        val novas = p.periciasClasseAutomaticas.toMutableList()
        while (novas.size <= index) novas.add("")
        val antiga = novas[index]
        if (antiga == nome) return
        val mapa = p.pericias.toMutableMap()
        novas[index] = ""
        if (antiga.isNotBlank() && antiga !in p.periciasAutomaticas && antiga !in novas) mapa.remove(antiga)
        novas[index] = nome
        if (nome.isNotBlank()) mapa[nome] = (mapa[nome] ?: Pericia()).copy(treino = 5)
        patch(p.copy(pericias = mapa, periciasClasseAutomaticas = novas))
    }

    Painel(titulo="Etapa 4 — Classe") {
        Selecao(p.classe, CLASSES.map{it.nome}, { aplicarClasse(it) }, placeholder="Escolha uma classe")
        classeAtual?.let { c ->
            Text(c.desc, color=TextoFraco)
            Text("Perícias da classe", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top = 8.dp))
            Text(c.pericias, color=TextoClaro, style=MaterialTheme.typography.bodySmall)
            Text(
                "Proficiências: " + when (c.nome) {
                    ...
                    else -> "consulte a progressão específica"
                },
                color=TextoFraco,
            ...
            when (c.nome) {
                "Combatente" -> {
                    Text("Escolha 1: Luta ou Pontaria", color=Acento, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 8.dp))
                    Selecao(
                        p.periciasClasseAutomaticas.getOrNull(0).orEmpty(),
                        listOf("Luta", "Pontaria").filter { it !in origemPericias },
                        { definirClassePericia(0, it) },
                        placeholder="Luta ou Pontaria"
                    )
                    Text("Escolha 1: Fortitude ou Reflexos", color=Acento, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 8.dp))
                    Selecao(
                        p.periciasClasseAutomaticas.getOrNull(1).orEmpty(),
                        listOf("Fortitude", "Reflexos").filter { it !in origemPericias },
                        { definirClassePericia(1, it) },
                        placeholder="Fortitude ou Reflexos"
                    )
                    Text("Depois, na Etapa 9, escolha mais ${1 + (p.atributos["int"] ?: 1)} perícia(s) à sua escolha.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                }
                "Ocultista" -> {
                    Text("A classe concede Ocultismo e Vontade. Se uma delas já vier da origem, escolha outra perícia para substituí-la.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 8.dp))
                    val obrigatorias = listOf("Ocultismo", "Vontade")
                    obrigatorias.forEachIndexed { index, base ->
                        val precisaSubstituir = base in origemPericias
                        if (!precisaSubstituir) {
                            Text("${base}: concedida pela classe", color=TextoClaro, style=MaterialTheme.typography.bodySmall)
                        } else {
                            Text("$base já veio da origem — escolha uma substituta", color=Acento, style=MaterialTheme.typography.bodySmall)
                            val opcoes = PERICIAS.map { it.nome }.filter { it !in origemPericias && it !in p.periciasClasseAutomaticas && it != base }
                            Selecao(
                                p.periciasClasseAutomaticas.getOrNull(index).orEmpty(),
                                opcoes,
                                { definirClassePericia(index, it) },
                                placeholder="Perícia substituta"
                            )
                        }
                    }
                    Text("Depois, na Etapa 9, escolha mais ${3 + (p.atributos["int"] ?: 1)} perícia(s) à sua escolha.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                }
                "Especialista" -> {
                    Text("Todas as perícias da classe são escolhidas livremente na Etapa 9: ${7 + (p.atributos["int"] ?: 1)} no total.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 8.dp))
                }
                "Sobrevivente" -> {
                    Text("As perícias dependem do estágio/regra do Sobrevivente. A seleção específica permanece na progressão dessa classe.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 8.dp))
                }
            }
        }
    }
...
private fun periciasIniciaisDaClasse(classe: String, origem: Set<String>, escolhas: List<String>): List<String> {
    return when (classe) {
        "Combatente" -> listOf("Luta", "Fortitude").mapIndexed { i, padrao ->
            escolhas.getOrNull(i)?.takeIf { it.isNotBlank() && it !in origem }
                ?: padrao.takeIf { it !in origem }
                ?: if (i == 0) "Pontaria" else "Reflexos"
        }
        "Ocultista" -> listOf("Ocultismo", "Vontade").mapIndexed { i, base ->
            escolhas.getOrNull(i)?.takeIf { it.isNotBlank() && it !in origem }
                ?: base.takeIf { it !in origem }
                ?: ""
        }
        else -> emptyList()
    }
...
private fun TrilhaCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    val poderesDaTrilha = remember(p.trilha) {
        PODERES_TRILHA_REGRAS.filter { it.trilha == p.trilha }.sortedBy { it.nexMin }
    }
    Painel(titulo="Etapa 6 — Trilha") {
        if(p.nex<10) {
            Text("A trilha é escolhida em NEX 10%. Você pode continuar sem trilha enquanto estiver abaixo desse NEX.", color=TextoFraco)
            Selecao(p.trilha, listOf("Nenhuma"), { patch(p.copy(trilha="")) }, placeholder="Nenhuma")
        } else {
            Selecao(if(p.trilha.isBlank()) "Nenhuma" else p.trilha, listOf("Nenhuma")+trilhas, {
                patch(p.copy(trilha=if(it=="Nenhuma") "" else it))
            }, placeholder="Escolha uma trilha")

            if (p.trilha.isNotBlank()) {
                Text("Visão geral", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=10.dp))
                Text(DESCRICOES_TRILHAS[p.trilha] ?: "Consulte a descrição completa no Compêndio.", color=TextoClaro, style=MaterialTheme.typography.bodySmall)
                Text("O que você ganha", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=10.dp))
                if (poderesDaTrilha.isEmpty()) {
                    Text("Os poderes desta trilha ainda não estão detalhados no catálogo atual. A escolha continua registrada, mas o app não inventa benefícios que não estejam catalogados.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                } else {
                    poderesDaTrilha.forEach { poder ->
                        val liberado = p.nex >= poder.nexMin
                        Column(Modifier.fillMaxWidth().padding(vertical = 6.dp)) {
                            Text("NEX ${poder.nexMin}% — ${poder.nome}", color=if (liberado) Acento else TextoFraco, style=MaterialTheme.typography.titleSmall)
                            Text(poder.descricao, color=TextoClaro, style=MaterialTheme.typography.bodySmall)
                            if (poder.requisito.isNotBlank()) Text("Pré-requisito: ${poder.requisito}", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                            if (poder.custo.isNotBlank()) Text("Custo: ${poder.custo}", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                            Text(if (liberado) "✓ Liberado neste NEX" else "🔒 Ainda será liberado em NEX ${poder.nexMin}%", color=if (liberado) Acento else TextoFraco, style=MaterialTheme.typography.labelSmall)
                        }
                    }
                }
                Text("A trilha não é um poder único: você recebe o primeiro poder em NEX 10% e novos poderes em NEX 40%, 65% e 99%.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top = 6.dp))
            } else {
                Text("Escolha uma trilha para ver o estilo de jogo, os poderes recebidos e em quais NEX eles são liberados.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
            }
        }
    }
}
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (etapaTroca == 0) {
                        ...
                        p.rituais
                            .filter { r ->
                                r.nome !in rituaisAutomaticos && (
                                    r.origem == "APRENDER_RITUAL" ||
                                        (r.origem == "DESCONHECIDA" && p.poderesParanormais.any { it.nome == "Aprender Ritual" && it.ritualNome == r.nome })
                                    )
                            }
                            .forEach { r ->
                                Button(
                                    ...
                                    Column(Modifier.fillMaxWidth()) {
                                        Text(r.nome, color = TextoClaro)
                                        Text("${r.circulo} círculo · ${r.elemento} · ${r.origem}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                                        Text(r.descricao, color = TextoFraco, style = MaterialTheme.typography.bodySmall, maxLines = 2)
                                    }
                                }
                            }
                        }
                        if (p.rituais.none { r -> r.nome !in rituaisAutomaticos && (r.origem == "APRENDER_RITUAL" || (r.origem == "DESCONHECIDA" && p.poderesParanormais.any { it.nome == "Aprender Ritual" && it.ritualNome == r.nome })) }) {
                        ...
                        Column(
                            ...
                            rituaisElegiveis
                                .filterNot { r -> ritualSelecionadoAntigo?.let { it.nome == r.nome && it.elemento == r.elemento && it.circulo == r.circulo } == true }
                                .forEach { r ->
                                    Button(
                                        onClick = {
                                            val novoRitual = Ritual(
                                                ...
                                                fonte = r.fonte,
                                                origem = "APRENDER_RITUAL",
                                            )
                                            if (ritualSelecionadoAntigo != null) {
                                                val antigo = ritualSelecionadoAntigo!!
                                                val novosRituais = p.rituais.map { existente -> if (existente.id == antigo.id) novoRitual else existente }
                                                val novosPoderes = selecionados.toMutableList().also { lista ->
                                                    if (slot in lista.indices && lista[slot].nome == "Aprender Ritual") {
                                                        lista[slot] = lista[slot].copy(elemento = novoRitual.elemento, ritualNome = novoRitual.nome)
                                                    }
                                                }
                                        ...
                                        Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(3.dp)) {
                                            Text("${r.simbolo} ${r.nome}", color = TextoClaro, style = MaterialTheme.typography.titleSmall)
                                            Text("${r.circulo} círculo · ${r.elemento} · ${r.fonte}", color = Acento, style = MaterialTheme.typography.labelSmall)
                                            Text("${r.execucao} · ${r.alcance}${if (r.resistencia.isNotBlank()) " · Resistência: ${r.resistencia}" else ""}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                                            Text(r.descricao, color = TextoFraco, style = MaterialTheme.typography.bodySmall, maxLines = 4)
                                            if (r.discenteDescricao.isNotBlank() || r.verdadeiroDescricao.isNotBlank()) {
                                                Text("Possui formas aprimoradas (Discente/Verdadeira)", color = Acento, style = MaterialTheme.typography.labelSmall)
                                            }
                                        }
                                    }
                                }
                        ...
                        Text(
                            if (ritualSelecionadoAntigo != null) "A quantidade total de rituais será mantida: você está trocando um ritual por outro."
                            else "Agora você vê detalhes suficientes para escolher conscientemente: círculo, elemento, origem, execução, alcance, resistência e resumo.",
                            color = Acento,
...
private fun PericiasCriacao(
    ...
    val automaticasOrigem = p.periciasAutomaticas.toSet()
    val automaticasClasse = p.periciasClasseAutomaticas.toSet()
    val bloqueadas = automaticasOrigem + automaticasClasse
    val selecionadasManuais = selecionadas.filter { it !in bloqueadas }.toSet()
    var emTroca by remember { mutableStateOf<String?>(null) }

    Painel(titulo="Etapa 9 — Perícias") {
        Text("Escolhas livres: ${selecionadasManuais.size}/$limite", color = Acento)
        if (automaticasOrigem.isNotEmpty()) Text("Origem: ${automaticasOrigem.joinToString(", ")}", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        if (automaticasClasse.isNotEmpty()) Text("Classe: ${automaticasClasse.joinToString(", ")}", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        Text(
            "Perícias da origem e da classe já concedidas ficam bloqueadas. Aqui você escolhe apenas as perícias livres restantes; uma escolha não pode ser usada duas vezes.",
            color = TextoFraco,
            style = MaterialTheme.typography.bodySmall,
        )
        if (emTroca != null) {
        ...
        PERICIAS.forEach { def ->
            val automatica = def.nome in bloqueadas
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                ...
                if (selecionada && !automatica) TextButton(onClick = { emTroca = def.nome }) { Text("Trocar") }
            }
        }
    }
}
