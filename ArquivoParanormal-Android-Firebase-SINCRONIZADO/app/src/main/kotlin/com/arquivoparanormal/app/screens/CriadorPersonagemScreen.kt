...
import com.arquivoparanormal.app.data.validarPersonagemV36
import com.arquivoparanormal.app.data.registrarAlteracao
import com.arquivoparanormal.app.data.periciasDisponiveisParaClasse
import com.arquivoparanormal.app.data.periciasClasseCumpreRegra
...
fun CriadorPersonagemScreen(
    ...
    var pericias by remember(personagem.id, personagem.pericias, personagem.periciasClasseEscolhidas, personagem.periciasAutomaticas) { mutableStateOf(personagem.pericias.filterValues { it.treino > 0 }.keys.toSet()) }
    ...
    fun patch(novo: Personagem) {
        ...
        val normalizado = novo.copy(
            patente = patentePorPrestigio(novo.prestigio),
            habilidades = (automaticos + manuais).distinct().joinToString("\n"),
            poderesAutomaticos = automaticos,
        )
        val auditado = if (anterior != null && anterior != normalizado) {
            val eventos = buildList {
                if (anterior.nex != normalizado.nex) add("NEX: ${anterior.nex}% → ${normalizado.nex}%")
                if (anterior.classe != normalizado.classe) add("Classe: ${anterior.classe} → ${normalizado.classe}")
                if (anterior.trilha != normalizado.trilha) add("Trilha: ${anterior.trilha.ifBlank { "nenhuma" }} → ${normalizado.trilha.ifBlank { "nenhuma" }}")
                if (anterior.prestigio != normalizado.prestigio) add("Prestígio: ${anterior.prestigio} → ${normalizado.prestigio}")
                if (anterior.rituais != normalizado.rituais) add("Lista de rituais alterada")
                if (anterior.habilidades != normalizado.habilidades) add("Poderes/habilidades alterados")
            }
            eventos.fold(normalizado) { acc, evento -> acc.registrarAlteracao("CRIACAO", evento) }
        } else normalizado
        repo.salvar(sincronizarRecursosDerivados(anterior, auditado))
    }
                ...
                when (etapa) {
                    ...
                    9 -> PericiasCriacao(
                        ...
                        onChange = { novoSet, antiga, nova ->
                            ...
                            if (antiga != null && antiga !in personagem.periciasClasseEscolhidas) mapa[antiga] = (mapa[antiga] ?: Pericia()).copy(treino = 0)
            ...
            Button(
                onClick = {
                    if (etapa == totalEtapas) {
                        val atual = repo.personagem(id)
                        if (atual != null) {
                            val v = validarPersonagemV36(atual)
                            if (v.valido) aoConcluir()
                            else aoConcluir() // validação também fica disponível no histórico/avisos sem bloquear fichas do Mestre
                        } else aoConcluir()
                    } else etapa++
                },
                modifier = Modifier.weight(1f),
...
private fun ClasseCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    Painel(titulo="Etapa 4 — Classe") {
        Selecao(p.classe, CLASSES.map{it.nome}, { nova ->
            val mapa = p.pericias.toMutableMap()
            p.periciasClasseEscolhidas.forEach { mapa[it] = (mapa[it] ?: Pericia()).copy(treino = 0) }
            patch(p.copy(classe=nova, trilha="", periciasClasseEscolhidas=emptyList(), pericias=mapa))
        ...
        CLASSES.firstOrNull{it.nome==p.classe}?.let { c ->
            ...
            if (c.nome == "Combatente") {
                val origem = p.periciasAutomaticas.toSet()
                val a = listOf("Luta", "Pontaria")
                val b = listOf("Fortitude", "Reflexos")
                val escA = p.periciasClasseEscolhidas.firstOrNull { it in a }
                val escB = p.periciasClasseEscolhidas.firstOrNull { it in b }
                LaunchedEffect(p.classe, p.periciasAutomaticas, p.periciasClasseEscolhidas) {
                    var escolhas = p.periciasClasseEscolhidas
                    if (origem.any { it in a }) {
                        escolhas = (escolhas.filterNot { it in a } + a.first { it !in origem }).distinct()
                    }
                    if (origem.any { it in b }) {
                        escolhas = (escolhas.filterNot { it in b } + b.first { it !in origem }).distinct()
                    }
                    if (escolhas != p.periciasClasseEscolhidas) {
                        val mapa = p.pericias.toMutableMap()
                        escolhas.forEach { mapa[it] = (mapa[it] ?: Pericia()).copy(treino = 5) }
                        patch(p.copy(periciasClasseEscolhidas = escolhas, pericias = mapa))
                    }
                }
                Text("O Combatente recebe Luta ou Pontaria, Fortitude ou Reflexos, mais 1 + Intelecto perícias à escolha. Se a origem já conceder uma dessas perícias, escolha a outra.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                Text("Escolha obrigatória 1 — Luta ou Pontaria", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=6.dp))
                if (origem.any { it in a }) {
                    val obrigatoria = a.first { it !in origem }
                    Chip("$obrigatoria — escolhida por causa da duplicata da origem", true, {}, Primaria, enabled=false)
                } else {
                    Row(horizontalArrangement=Arrangement.spacedBy(6.dp)) { a.forEach { n -> Chip(n, escA==n, { run { val escolhas=(p.periciasClasseEscolhidas.filterNot { it in a } + n).distinct(); val mapa=p.pericias.toMutableMap(); mapa[n]=(mapa[n] ?: Pericia()).copy(treino=5); patch(p.copy(periciasClasseEscolhidas=escolhas, pericias=mapa)) } }, Primaria) } }
                }
                Text("Escolha obrigatória 2 — Fortitude ou Reflexos", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=6.dp))
                if (origem.any { it in b }) {
                    val obrigatoria = b.first { it !in origem }
                    Chip("$obrigatoria — escolhida por causa da duplicata da origem", true, {}, Primaria, enabled=false)
                } else {
                    Row(horizontalArrangement=Arrangement.spacedBy(6.dp)) { b.forEach { n -> Chip(n, escB==n, { run { val escolhas=(p.periciasClasseEscolhidas.filterNot { it in b } + n).distinct(); val mapa=p.pericias.toMutableMap(); mapa[n]=(mapa[n] ?: Pericia()).copy(treino=5); patch(p.copy(periciasClasseEscolhidas=escolhas, pericias=mapa)) } }, Primaria) } }
                }
                Text("Depois, na Etapa 9, você escolherá mais ${1 + (p.atributos["int"] ?: 1)} perícia(s) à sua escolha.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top=6.dp))
            }
            if (c.nome == "Sobrevivente") Text("⚠ Sobrevivente usa Estágios no PDF, não a progressão normal por NEX. A ficha comum por NEX não deve ser usada para simular os Estágios dessa classe.", color=Acento, style=MaterialTheme.typography.bodySmall)
        }
    }
}
...
private fun NexCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    Painel(titulo="Etapa 5 — NEX e Prestígio") {
        Selecao(p.nex.toString(), nexOptions, { patch(p.copy(nex=it.toInt())) }, placeholder="Escolha o NEX")
        Text("NEX mede a exposição paranormal. Ele não determina sua patente.", color=TextoFraco)
        Text("Prestígio (PP)", color=Acento, style = MaterialTheme.typography.labelSmall)
        Numero(p.prestigio) { novo -> val pp = novo.coerceAtLeast(0); patch(p.copy(prestigio = pp, patente = patentePorPrestigio(pp))) }
        Text("Patente calculada: ${patentePorPrestigio(p.prestigio)}", color=Acento)
        Text("0 PP Recruta · 20 PP Operador · 50 PP Agente Especial · 100 PP Oficial de Operações · 200 PP Agente de Elite.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
    }
}
...
private fun TrilhaCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    val poderesTrilha = remember(p.classe, p.trilha) {
        com.arquivoparanormal.app.data.PODERES_TRILHA_REGRAS
            .filter { it.trilha == p.trilha && (it.classe.isBlank() || it.classe.split("/").contains(p.classe)) }
            .sortedBy { it.nexMin }
    }
    var detalheTrilha by remember(p.trilha) { mutableStateOf<String?>(null) }
    Painel(titulo = "Etapa 6 — Trilha") {
        Text("A trilha define a especialização do personagem. Ela é escolhida em NEX 10% e concede uma habilidade em 10%, 40%, 65% e 99%.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        if (p.nex < 10) {
            Text("Você ainda não atingiu NEX 10%. A escolha da trilha ficará disponível quando chegar a esse NEX.", color=Acento, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top=6.dp))
            Selecao(p.trilha, listOf("Nenhuma"), { patch(p.copy(trilha="")) }, placeholder="Nenhuma")
        } else {
            Selecao(if(p.trilha.isBlank()) "Nenhuma" else p.trilha, listOf("Nenhuma")+trilhas, { escolha -> patch(p.copy(trilha=if(escolha=="Nenhuma") "" else escolha)) }, placeholder="Escolha uma trilha")
            if (p.trilha.isBlank()) {
                Text("Selecione uma trilha para ver exatamente o que ela oferece em cada NEX. Você poderá comparar os benefícios antes de confirmar a escolha.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top=8.dp))
            } else {
                Text(p.trilha, color=Acento, style=MaterialTheme.typography.titleMedium, modifier=Modifier.padding(top=10.dp))
                Text(DESCRICOES_TRILHAS[p.trilha] ?: "Descrição resumida indisponível para esta trilha.", color=TextoFraco, style=MaterialTheme.typography.bodySmall, modifier=Modifier.padding(top=3.dp))
                Text("O que esta trilha oferece", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=10.dp,bottom=4.dp))
                if (poderesTrilha.isEmpty()) {
                    Text("As habilidades desta trilha ainda não estão catalogadas nesta versão. A escolha não será tratada como uma habilidade disponível até que ela seja catalogada.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                } else {
                    poderesTrilha.forEach { poder ->
                        val liberado=p.nex>=poder.nexMin; val atual=p.nex==poder.nexMin
                        Column(modifier=Modifier.fillMaxWidth().padding(vertical=4.dp)) {
                            Row(modifier=Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween) {
                                Text("NEX ${poder.nexMin}% — ${poder.nome}", color=if(liberado) TextoClaro else TextoFraco, style=MaterialTheme.typography.bodyMedium)
                                Text(if(atual) "AGORA" else if(liberado) "LIBERADO" else "BLOQUEADO", color=if(liberado) Acento else TextoFraco, style=MaterialTheme.typography.labelSmall)
                            }
                            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp), verticalAlignment = Alignment.CenterVertically) {
                                Text(
                                    poder.descricao,
                                    color=if(liberado) TextoClaro else TextoFraco,
                                    style=MaterialTheme.typography.bodySmall,
                                    maxLines=2,
                                    modifier=Modifier.weight(1f).padding(top=2.dp)
                                )
                                TextButton(onClick = { detalheTrilha = poder.nome }) { Text("Ver detalhes") }
                            }
                        }
                    }
                }
                Text("Importante: as habilidades mostradas aqui são os benefícios da trilha; atingir o NEX libera a habilidade conforme a progressão. A escolha da trilha não concede automaticamente poderes opcionais que tenham de ser escolhidos separadamente.", color=TextoFraco, style=MaterialTheme.typography.labelSmall, modifier=Modifier.padding(top=8.dp))
            }
        }
    }

    detalheTrilha?.let { nome ->
        val habilidade = poderesTrilha.firstOrNull { it.nome == nome }
        if (habilidade != null) {
            AlertDialog(
                onDismissRequest = { detalheTrilha = null },
                title = { Text("${habilidade.nome} — NEX ${habilidade.nexMin}%") },
                text = {
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text("Como funciona", color = Acento, style = MaterialTheme.typography.titleSmall)
                        Text(habilidade.descricao, color = TextoClaro)
                        Text("Esta é a descrição mecânica resumida para a ficha. Para a redação integral, consulte o livro indicado na ficha da habilidade.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        if (habilidade.requisito.isNotBlank()) Text("Requisito: ${habilidade.requisito}", color = Acento, style = MaterialTheme.typography.labelSmall)
                        Text("Fonte: ${habilidade.livro}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    }
                },
                confirmButton = { TextButton(onClick = { detalheTrilha = null }) { Text("Fechar") } }
            )
        }
    }
}
...
@Composable
internal fun PoderesParanormaisCriacao(
    p: Personagem,
    vagas: Int,
    ...
    var poderElementoTrocaSlot by remember { mutableStateOf<Int?>(null) }
    val maxCirculo = if (p.nex >= 75) 3 else if (p.nex >= 45) 2 else 1
    val aprendidosPorPoder = selecionados.count { it.nome == "Aprender Ritual" }
    val limiteAprenderRitual = (p.atributos["int"] ?: 0).coerceAtLeast(0)
    ...
    Text("Aprender Ritual: ${aprendidosPorPoder}/$limiteAprenderRitual pelo limite global de Intelecto. Substituir um ritual não consome novo slot.", color = if (aprendidosPorPoder >= limiteAprenderRitual) Perigo else TextoFraco, style = MaterialTheme.typography.bodySmall)
    ...
    exibidos.forEach { poder ->
        ...
        val limiteGlobalAprender = poder.nome != "Aprender Ritual" || escolhidoNormal || aprendidosPorPoder < limiteAprenderRitual
        val pode = selecionados.size < vagasTotais && limiteGlobalAprender && (!escolhidoNormal || poder.nome == "Aprender Ritual") && !escolhidoAfinidade
        ...
        Column(Modifier.fillMaxWidth().animateContentSize().padding(vertical = 4.dp)) {
            Chip(
                ...
                aoClicar = {
                    when (poder.nome) {
                        ...
                        "Resistir a Elemento" -> if (pode) { poderElementoDialog = poder; poderElementoTrocaSlot = null }
                        else -> {
            ...
            if (escolhidoNormal || escolhidoAfinidade) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    if (poder.nome == "Resistir a Elemento" && escolhidoNormal) {
                        val idx = selecionados.indexOfFirst { it.nome == poder.nome && !it.afinidade }
                        if (idx >= 0) {
                            TextButton(onClick = { poderElementoDialog = poder; poderElementoTrocaSlot = idx }) {
                                Text("Trocar elemento")
                            }
                        }
                    }
                    TextButton(onClick = {
    ...
    poderElementoDialog?.let { poder ->
        val trocando = poderElementoTrocaSlot != null && poderElementoTrocaSlot!! in selecionados.indices
        AlertDialog(
            onDismissRequest = { poderElementoDialog = null; poderElementoTrocaSlot = null },
            title = { Text(if (trocando) "Trocar elemento" else "Escolha o elemento") },
            text = {
                Column {
                    if (trocando) {
                        Text(
                            "Elemento atual: ${selecionados[poderElementoTrocaSlot!!].elemento}. Escolha o novo elemento para ${poder.nome}.",
                            color = TextoFraco,
                            style = MaterialTheme.typography.bodySmall,
                    ...
                    listOf("Conhecimento", "Energia", "Morte", "Sangue").forEach { elemento ->
                        val usado = selecionados.filterIndexed { idx, _ -> idx != poderElementoTrocaSlot }
                            .any { it.nome == poder.nome && it.elemento == elemento }
                        Button(onClick = {
                            if (!usado) {
                                val novaLista = if (trocando) {
                                    selecionados.toMutableList().also { it[poderElementoTrocaSlot!!] = it[poderElementoTrocaSlot!!].copy(elemento = elemento) }
                                } else {
                                    selecionados + PoderParanormalSelecionado(poder.nome, elemento)
                                }
                                onChange(novaLista, p.rituais)
                                poderElementoDialog = null
                                poderElementoTrocaSlot = null
                            }
                        }, enabled = !usado, modifier = Modifier.fillMaxWidth()) { Text(if (usado) "$elemento — já escolhido" else elemento) }
                    }
                }
            },
            confirmButton = { TextButton(onClick = { poderElementoDialog = null; poderElementoTrocaSlot = null }) { Text("Cancelar") } },
        )
    }
        ...
        AlertDialog(
            ...
            text = {
                Column(
                    ...
                    if (etapaTroca == 0) {
                        Text("O que o livro determina", color = Acento, style = MaterialTheme.typography.titleSmall)
                        Text("Qualquer personagem pode aprender rituais por meio de Aprender Ritual. O número de rituais aprendidos dessa forma não pode ultrapassar seu Intelecto. O poder pode ser escolhido várias vezes, mas cada novo ritual consome uma dessas vagas. Rituais obtidos por habilidades de classe não entram nesse limite.", color = TextoClaro, style = MaterialTheme.typography.bodySmall)
                        Text("Círculo máximo: até 1º círculo antes de NEX 45%; até 2º círculo a partir de NEX 45%; até 3º círculo a partir de NEX 75%.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        Text("Também é possível substituir um ritual que você já conhece por outro elegível. A substituição troca o ritual, sem aumentar a quantidade total.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        Text("O ritual escolhido passa a contar como um poder do elemento dele para requisitos de poderes paranormais.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        ...
                        Text("Regra do livro: Aprender Ritual usa o limite de rituais aprendidos igual ao seu Intelecto. Nesta etapa você está escolhendo o ritual associado a este poder; rituais concedidos por classe/trilha são tratados separadamente.", color = Acento, style = MaterialTheme.typography.labelSmall)
                        val catalogoParaConsulta = RITUAIS_COMPLETOS
                            .filter { r ->
                                val n = when (r.circulo) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
                                n <= maxCirculoPermitido
                            }
                            .filterNot { r -> ritualSelecionadoAntigo?.let { it.nome == r.nome && it.elemento == r.elemento } == true }
                            .distinctBy { ritual -> "${ritual.nome}|${ritual.elemento}|${ritual.circulo}" }
                        // Dentro deste diálogo o personagem está literalmente no processo de obter
                        // "Aprender Ritual" (o poder só é gravado na ficha ao final, junto do ritual
                        // escolhido). Por isso, para fins de checagem de bloqueio, tratamos o poder
                        // como já concedido — senão motivoBloqueioRitual() bloqueia todo o catálogo
                        // para quem ainda não é Ocultista e está escolhendo Aprender Ritual pela
                        // primeira vez (ex.: Especialista/Combatente via Transcender).
                        val pParaValidacao = if (p.poderesParanormais.any { it.nome == "Aprender Ritual" }) p
                            else p.copy(poderesParanormais = p.poderesParanormais + PoderParanormalSelecionado("Aprender Ritual", ""))
                        Text("Todos os rituais do catálogo até o círculo permitido aparecem aqui. Rituais bloqueados continuam visíveis para consulta, com o motivo do bloqueio. Toque em \"Ver detalhes\" para ler a descrição completa antes de escolher.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        catalogoParaConsulta.forEach { r ->
                            val bloqueio = motivoBloqueioRitual(pParaValidacao, r)
                            val podeEscolher = bloqueio == null
                            var detalhesRitualAberto by remember(r.nome, r.elemento, r.circulo) { mutableStateOf(false) }
                            val escolherEsteRitual = {
                                val novoRitual = Ritual(
                                    ...
                                    fonte = fonteDoRitual(r), origem = "APRENDER_RITUAL",
                                )
                            ...
                            Column(Modifier.fillMaxWidth().padding(vertical = 3.dp)) {
                                Text(
                                    "${if (podeEscolher) "" else "🔒 "}${r.nome} · ${r.circulo} · ${r.elemento}",
                                    color = if (podeEscolher) TextoClaro else TextoFraco,
                                    style = MaterialTheme.typography.bodyMedium,
                                    modifier = Modifier.padding(start = 8.dp),
                                )
                                Text(r.descricao, color = TextoFraco, style = MaterialTheme.typography.bodySmall, maxLines = 2, modifier = Modifier.padding(start = 8.dp))
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                    TextButton(onClick = { detalhesRitualAberto = true }) { Text("Ver detalhes") }
                                    Button(
                                        onClick = { if (podeEscolher) escolherEsteRitual() },
                                        enabled = podeEscolher,
                                    ) { Text(if (ritualSelecionadoAntigo != null) "Trocar" else "Escolher") }
                                }
                                if (!podeEscolher) Text(bloqueio ?: "Bloqueado", color = Perigo, style = MaterialTheme.typography.labelSmall, modifier = Modifier.padding(start = 8.dp))
                            }
                            if (detalhesRitualAberto) {
                                AlertDialog(
                                    onDismissRequest = { detalhesRitualAberto = false },
                                    title = { Text("${r.nome} · ${r.circulo} · ${r.elemento}") },
                                    text = {
                                        Column(
                                            modifier = Modifier.heightIn(max = 420.dp).verticalScroll(rememberScrollState()),
                                            verticalArrangement = Arrangement.spacedBy(6.dp),
                                        ) {
                                            Text(r.descricao, color = TextoClaro, style = MaterialTheme.typography.bodyMedium)
                                            Text("${r.execucao} · ${r.alcance} · ${r.duracao}${if (r.resistencia.isNotBlank()) " · ${r.resistencia}" else ""}", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            if (r.alvo.isNotBlank()) Text("Alvo: ${r.alvo}", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            if (r.area.isNotBlank()) Text("Área: ${r.area}", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            Text("Custo: ${r.custoPE} PE", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            if (r.discenteDescricao.isNotBlank()) Text("Discente: ${r.discenteDescricao}", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            if (r.verdadeiroDescricao.isNotBlank()) Text("Verdadeiro: ${r.verdadeiroDescricao}", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                                            Text("Fonte: ${fonteDoRitual(r)}", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                                            if (!podeEscolher) Text(bloqueio ?: "Bloqueado", color = Perigo, style = MaterialTheme.typography.labelSmall)
                                        }
                                    },
                                    confirmButton = {
                                        TextButton(onClick = { escolherEsteRitual() }, enabled = podeEscolher) {
                                            Text(if (ritualSelecionadoAntigo != null) "Trocar" else "Escolher")
                                        }
                                    },
                                    dismissButton = { TextButton(onClick = { detalhesRitualAberto = false }) { Text("Fechar") } },
                                )
                            }
                        }
                        Text(
...
private fun RituaisCriacao(p: Personagem, patch: (Personagem) -> Unit) {
    ...
    val conhecidosDeClasse = p.rituais.count { it.origem == "CLASSE_OCULTISTA" }
            ...
            val adicionados = faltantes.mapNotNull { nome ->
                RITUAIS_COMPLETOS.firstOrNull { it.nome == nome }?.let { def ->
                    Ritual(
                        ...
                        fonte = fonteDoRitual(def), origem = "TRILHA",
                    )
                }
            }
                        ...
                        selecionado,
                        {
                            if (selecionado) {
                                ...
                                patch(p.copy(rituais = p.rituais + Ritual(
                                    ...
                                    discenteExtraPE = def.discenteExtraPE, verdadeiroExtraPE = def.verdadeiroExtraPE,
                                    requisitoDiscente = def.requisitoDiscente, requisitoVerdadeiro = def.requisitoVerdadeiro,
                                    fonte = fonteDoRitual(def), origem = "CLASSE_OCULTISTA",
                                )))
                            }
                        },
...
@Composable
private fun PericiasCriacao(p: Personagem, selecionadas: Set<String>, onChange: (Set<String>, String?, String?) -> Unit) {
    val origem = p.periciasAutomaticas.toSet()
    val classeFixas = p.periciasClasseEscolhidas.toSet()
    val limiteExtras = when (p.classe) {
    ...
    val manuais = selecionadas.filter { it !in origem }.toSet()
    val extras = manuais - classeFixas
    var emTroca by remember { mutableStateOf<String?>(null) }
    Painel(titulo="Etapa 9 — Perícias") {
        Text("Escolhidas: ${extras.size}/$limiteExtras adicionais · ${classeFixas.size} da classe · ${origem.size} da origem", color=Acento)
        Text(if (p.classe=="Combatente") "As escolhas obrigatórias do Combatente foram feitas na Etapa 4. Agora escolha mais 1 + Intelecto perícias. Origem e perícias da classe ficam bloqueadas." else "Perícias da origem e da classe ficam registradas; complete aqui as escolhas adicionais permitidas pela classe.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        if (emTroca != null) Text("Escolha uma nova perícia para substituir $emTroca.", color=Acento, style=MaterialTheme.typography.bodySmall, modifier = Modifier.animateContentSize())
        PERICIAS.forEach { def ->
            val auto=def.nome in origem; val fixa=def.nome in classeFixas; val escolhida=def.nome in extras
            val pode=!auto && !fixa && (emTroca!=null || escolhida || extras.size<limiteExtras)
            Row(Modifier.fillMaxWidth().animateContentSize(), horizontalArrangement=Arrangement.SpaceBetween) {
                Chip("${def.nome} (${def.attr.uppercase()})", auto||fixa||escolhida, {
                    if (emTroca!=null && pode) { val novo=(manuais-emTroca!!+def.nome); onChange(novo,emTroca,def.nome); emTroca=null }
                    else if (!escolhida && pode) onChange(manuais+def.nome,null,def.nome)
                }, if(auto||fixa) TextoFraco else Primaria, enabled=pode)
                if (escolhida) TextButton(onClick={emTroca=def.nome}){Text("Trocar")}
            }
        }
        Text("Total de perícias treinadas será calculado somando origem + classe + escolhas adicionais, evitando duplicatas conforme a regra.", color=TextoFraco, style=MaterialTheme.typography.labelSmall)
    }
}
...
private fun EquipamentosCriacao(p: Personagem, selecionadas:Set<String>, onToggle:(String)->Unit) {
    ...
    val limite = limiteDeCarga(p.atributos["for"] ?: 1, p.atributos["int"] ?: 1, tecnico)
...
private fun RevisaoCriacao(p: Personagem, onFinish:()->Unit) {
    Painel(titulo="Etapa 11 — Revisão") {
        ...
        val validacao = validarPersonagemV36(p)
        Text("Validação: ${if (validacao.valido) "OK" else "há erros"}", color = if (validacao.valido) Acento else androidx.compose.ui.graphics.Color(0xFFFF6B6B))
        validacao.erros.forEach { Text("• $it", color = androidx.compose.ui.graphics.Color(0xFFFF6B6B), style = MaterialTheme.typography.bodySmall) }
        validacao.avisos.forEach { Text("• $it", color = TextoFraco, style = MaterialTheme.typography.bodySmall) }
