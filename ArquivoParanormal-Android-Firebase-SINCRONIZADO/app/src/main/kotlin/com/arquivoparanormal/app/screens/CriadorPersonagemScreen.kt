...
private fun ClasseCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    Painel(titulo="Etapa 4 — Classe") {
        ...
        CLASSES.firstOrNull{it.nome==p.classe}?.let { c ->
            ...
            if (c.nome == "Combatente") Text("Na etapa de perícias você escolherá 1 + INT entre Atletismo, Fortitude, Luta, Pontaria, Reflexos e Vontade; pelo menos uma escolha deve ser Luta ou Pontaria.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
    ...
    Painel(titulo="Etapa 6 — Trilha") {
        if(p.nex<10) {
            ...
            if (p.trilha.isNotBlank()) {
                ...
                val poderesTrilha = com.arquivoparanormal.app.data.PODERES_TRILHA_REGRAS.filter { it.trilha == p.trilha && (it.classe.isBlank() || it.classe.split("/").contains(p.classe)) }.sortedBy { it.nexMin }
                if (poderesTrilha.isNotEmpty()) {
                    Text("O que você recebe e o que será liberado", color=Acento, style=MaterialTheme.typography.titleSmall, modifier=Modifier.padding(top=8.dp))
                    poderesTrilha.forEach { poder ->
                        val liberado = p.nex >= poder.nexMin
                        Text("${if (liberado) "✓" else "🔒"} NEX ${poder.nexMin}% — ${poder.nome}: ${poder.descricao}", color = if (liberado) TextoClaro else TextoFraco, style = MaterialTheme.typography.bodySmall, modifier = Modifier.padding(vertical = 2.dp))
                    }
                }
            } else {
...
private fun PericiasCriacao(
    ...
    val disponiveisClasse = periciasDisponiveisParaClasse(p.classe)
    ...
    Painel(titulo = "Etapa 9 — Perícias") {
        ...
        Text(
            if (p.classe == "Combatente") "Combatente: escolha 1 + INT entre as perícias da classe; pelo menos uma deve ser Luta ou Pontaria. Perícias de origem ficam bloqueadas." else "Perícias concedidas pela origem ficam cinzas e bloqueadas. As que você escolheu manualmente podem ser trocadas a qualquer momento.",
            color = TextoFraco,
            style = MaterialTheme.typography.bodySmall,
        )
        if (emTroca != null) {
        ...
        PERICIAS.forEach { def ->
            ...
            val foraDaClasse = def.nome !in disponiveisClasse
            val podeEscolher = !automatica && !foraDaClasse && (emTroca != null || selecionada || selecionadasManuais.size < limite)
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Chip(
        ...
        if (p.classe == "Combatente" && selecionadasManuais.isNotEmpty() && !periciasClasseCumpreRegra(p.classe, selecionadasManuais)) {
            Text("⚠ Escolha pelo menos Luta ou Pontaria para cumprir a regra do Combatente.", color = Perigo, style = MaterialTheme.typography.bodySmall)
        }
    }
}
