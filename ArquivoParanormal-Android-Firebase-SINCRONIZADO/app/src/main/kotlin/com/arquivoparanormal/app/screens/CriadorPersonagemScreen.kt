            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (etapaTroca == 0) {
                        ...
                        Text("Todos os rituais do catálogo até o círculo permitido aparecem aqui. Rituais bloqueados continuam visíveis para consulta, com o motivo do bloqueio.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                        catalogoParaConsulta.forEach { r ->
                            val bloqueio = motivoBloqueioRitual(p, r)
                            val podeEscolher = bloqueio == null
                            Column(Modifier.fillMaxWidth().padding(vertical = 3.dp)) {
                                Button(
                                    onClick = { if (podeEscolher) {
                                ...
                                ) { Text("${if (podeEscolher) "${r.nome} · ${r.circulo} · ${r.elemento}" else "🔒 ${r.nome} · ${r.circulo} · ${r.elemento}"}") }
                                Text(r.descricao, color = TextoClaro, style = MaterialTheme.typography.bodySmall, maxLines = 3, modifier = Modifier.padding(start = 8.dp))
                                Text("${r.execucao} · ${r.alcance} · ${r.duracao}${if (r.resistencia.isNotBlank()) " · ${r.resistencia}" else ""} · ${fonteDoRitual(r)}", color = TextoFraco, style = MaterialTheme.typography.labelSmall, modifier = Modifier.padding(start = 8.dp))
