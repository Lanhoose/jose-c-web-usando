...
private fun ClasseCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    Painel(titulo="Etapa 4 — Classe") {
        Selecao(p.classe, CLASSES.map{it.nome}, {
            if (it == "Sobrevivente") patch(p.copy(classe=it, trilha="", nex=0, prestigio=0, patente="Recruta"))
            else patch(p.copy(classe=it, trilha="", nex=5, patente=patentePorPrestigio(p.prestigio)))
...
private fun NexCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    Painel(titulo="Etapa 5 — ${if (p.classe == "Sobrevivente") "Estágio" else "NEX"}") {
        if (p.classe == "Sobrevivente") {
            Text("Sobrevivente começa no NEX 0% e evolui por Estágios. No fim de cada missão, sobe um estágio; não use a progressão normal de NEX enquanto permanecer Sobrevivente.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
            Selecao(p.estagioSobrevivente.toString(), (1..5).map { it.toString() }, { patch(p.copy(estagioSobrevivente = it.toInt(), nex = 0)) }, placeholder="Estágio")
            Text("Estágio atual: ${p.estagioSobrevivente}", color=Acento)
            Text(when (p.estagioSobrevivente) {
                1 -> "Empenho · limite de PE 1"
                2 -> "Trilha: escolha Durão, Esperto ou Esotérico"
                3 -> "Aumento de atributo +1"
                4 -> "Segundo poder da trilha"
                else -> "Cicatrizado"
            }, color=TextoClaro, style=MaterialTheme.typography.bodySmall)
            if (p.estagioSobrevivente >= 3 && p.atributosEscolhidos.none { it.startsWith("S3:") }) {
                Text("Estágio 3 — Aumento de atributo", color=Acento, style=MaterialTheme.typography.labelLarge)
                Row(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
                    ATRIBUTOS.forEach { a ->
                        Chip(a.sigla, false, {
                            val atual = p.atributos[a.key] ?: 1
                            if (atual < 3) patch(p.copy(atributos = p.atributos + (a.key to (atual + 1)), atributosEscolhidos = (p.atributosEscolhidos + "S3:${a.key}").distinct()))
                        }, Primaria)
                    }
                }
            }
            if (p.estagioSobrevivente >= 5) {
                Text("Estágio 5 — Cicatrizado", color=Acento, style=MaterialTheme.typography.labelLarge)
                Selecao(p.perigoCicatrizado, ELEMENTOS_AFINIDADE, { patch(p.copy(perigoCicatrizado = it)) }, placeholder="Perigo/elemento do trauma")
            }
            if (p.estagioSobrevivente >= 5 && p.treinamentoEspecialSobrevivente.isBlank()) {
                Text("Treinamento Especial — quando a história justificar, escolha a classe para a próxima subida.", color=Acento, style=MaterialTheme.typography.labelLarge)
                Selecao("", listOf("Combatente", "Especialista", "Ocultista"), { classeNova ->
                    patch(p.copy(classe = classeNova, nex = 5, treinamentoEspecialSobrevivente = classeNova, trilha = "", patente = patentePorPrestigio(p.prestigio)))
                }, placeholder="Escolher classe do treinamento especial")
            }
        } else {
            val nexOptions=(5..95 step 5).map{it.toString()} + "99"
            Selecao(p.nex.toString(), nexOptions, { patch(p.copy(nex=it.toInt(), patente=patentePorPrestigio(p.prestigio))) }, placeholder="Escolha o NEX")
            Text("NEX mede a exposição paranormal. Um agente iniciante começa em 5%.", color=TextoFraco)
        }
        Campo("Pontos de Prestígio") { Numero(p.prestigio) { patch(p.copy(prestigio = it.coerceAtLeast(0), patente = patentePorPrestigio(it.coerceAtLeast(0)))) } }
        Text("Patente atual: ${patentePorPrestigio(p.prestigio)} — independente do NEX.", color=Acento, style=MaterialTheme.typography.labelSmall)
    }
}
...
private fun TrilhaCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    Painel(titulo="Etapa 6 — Trilha") {
        if (p.classe == "Sobrevivente") {
            if (p.estagioSobrevivente < 2) {
                Text("A trilha de Sobrevivente só é escolhida no Estágio 2.", color=TextoFraco)
            } else {
                ...
                Text("Durão, Esperto e Esotérico possuem o primeiro poder no Estágio 2 e o segundo no Estágio 4.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
            }
        } else if(p.nex<10) {
            Text("A trilha é escolhida a partir de NEX 10%.", color=TextoFraco)
        ...
        if (p.trilha.isNotBlank()) {
            ...
            if (p.trilha.startsWith("Monstruoso")) {
                Text("Elemento da trilha Monstruoso", color=Acento, style=MaterialTheme.typography.labelLarge)
                Text("Esta escolha é independente da Conexão Elemental de NEX 50%.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
                Row(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
                    ELEMENTOS_AFINIDADE.forEach { e -> Chip(e, p.elementoMonstruoso == e, { patch(p.copy(elementoMonstruoso = e)) }, Primaria) }
                }
            }
        }
    }
...
private fun RituaisCriacao(p: Personagem, patch: (Personagem) -> Unit) {
    ...
    val selecionados = p.rituais.filter { it.origem == "CLASSE_OCULTISTA" || it.origem == "TRILHA" }.map { it.nome }.toSet()
        ...
        if (faltantes.isNotEmpty()) {
            val adicionados = faltantes.mapNotNull { nome ->
                RITUAIS_COMPLETOS.firstOrNull { it.nome == nome }?.let { def ->
                    Ritual(
                        origem = if (automaticos.contains(def.nome)) "TRILHA" else "CLASSE_OCULTISTA",
    ...
    Painel(titulo = "Etapa 8 — Rituais e Elementos") {
        ...
        if (p.classe == "Sobrevivente" && p.trilha == "Esotérico" && p.estagioSobrevivente >= 4) {
            Text("Iniciado — 1 ritual de 1º círculo", color = Acento, style = MaterialTheme.typography.titleSmall)
            Text("Este ritual é uma aquisição própria do Sobrevivente e permanece quando houver Treinamento Especial para Ocultista.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
            val iniciadoAtual = p.rituais.firstOrNull { it.origem == "SOBREVIVENTE_INICIADO" }
            val opcoes = RITUAIS_COMPLETOS.filter { it.circulo == "1º" }.distinctBy { "${it.nome}|${it.elemento}" }
            Selecao(iniciadoAtual?.let { "${it.nome} — ${it.elemento}" } ?: "", opcoes.map { "${it.nome} — ${it.elemento}" }, { escolha ->
                val def = opcoes.firstOrNull { "${it.nome} — ${it.elemento}" == escolha }
                if (def != null) {
                    val ritual = Ritual(origem = "SOBREVIVENTE_INICIADO", nome = def.nome, circulo = def.circulo, elemento = def.elemento, execucao = def.execucao, alcance = def.alcance, alvo = def.alvo, area = def.area, efeitoCampo = def.efeito, duracao = def.duracao, afinidade = def.afinidade, simbolo = def.simbolo, descricao = def.descricao, efeito = def.efeito, discenteDescricao = def.discenteDescricao, verdadeiroDescricao = def.verdadeiroDescricao, resistencia = def.resistencia, custoPE = def.custoPE, discenteExtraPE = def.discenteExtraPE, verdadeiroExtraPE = def.verdadeiroExtraPE, requisitoDiscente = def.requisitoDiscente, requisitoVerdadeiro = def.requisitoVerdadeiro)
                    patch(p.copy(rituais = p.rituais.filterNot { it.origem == "SOBREVIVENTE_INICIADO" } + ritual))
                }
            }, placeholder = "Escolha o ritual de Iniciado")
        } else if (!podeEscolherRitual) {
