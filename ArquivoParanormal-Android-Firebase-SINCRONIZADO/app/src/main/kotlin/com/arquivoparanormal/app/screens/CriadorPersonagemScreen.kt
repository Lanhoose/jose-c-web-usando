...
private fun NexCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    Painel(titulo="Etapa 5 — NEX e Prestígio") {
        ...
        Text("NEX mede a exposição paranormal e não determina sua patente.", color=TextoFraco)
        Campo("Pontos de Prestígio") { Numero(p.prestigio.coerceAtLeast(0)) { novo ->
            patch(p.copy(prestigio = novo.coerceAtLeast(0), patente = patentePorPrestigio(novo.coerceAtLeast(0))))
        } }
        Text("Patente atual: ${patentePorPrestigio(p.prestigio)}", color=Acento)
    }
}
