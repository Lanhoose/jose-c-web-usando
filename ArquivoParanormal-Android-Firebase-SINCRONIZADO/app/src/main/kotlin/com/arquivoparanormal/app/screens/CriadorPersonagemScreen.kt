            ...
            item {
                when (etapa) {
                    ...
                    8 -> PericiasCriacao(p = personagem, selecionadas = pericias, onToggle = { nome ->
                        ...
                        if (nome !in pericias) mapa[nome] = mapa[nome]!!.copy(treino = 0)
...
private fun AtributosCriacao(p: Personagem, patch: ((Personagem)->Unit)) {
    ...
    val limite = 4 + if (reduzidoAZero == 1) 1 else 0
    Painel(titulo = "Etapa 2 — Atributos") {
        Text("Comece com 1 em cada atributo e distribua 4 pontos. Um único atributo pode ser reduzido a 0 para ganhar +1 ponto. O máximo inicial é 3.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        ...
        ATRIBUTOS.forEach { a ->
            Row(Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween) {
                ...
                Row(horizontalArrangement=Arrangement.spacedBy(6.dp)) {
                    ...
                    Chip("+", true, { val v=p.atributos[a.key] ?: 1; val pontos=p.atributos.values.sumOf{(it-1).coerceAtLeast(0)}; val zeros=p.atributos.values.count{it==0}; val orcamento=4+if(zeros==1) 1 else 0; if(v<3 && pontos<orcamento) patch(p.copy(atributos=p.atributos+(a.key to (v+1)))) }, Primaria)
                }
            }
        }
    }
...
private fun PericiasCriacao(p: Personagem, selecionadas:Set<String>, onToggle:(String)->Unit) {
    ...
    Painel(titulo="Etapa 8 — Perícias") {
        Text("Escolhidas: ${selecionadas.size}/$limite (sem contar as perícias concedidas pela origem).", color=Acento)
        Text("Selecione as perícias de classe. Se a origem repetir uma perícia, escolha outra conforme as regras.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        PERICIAS.forEach { def -> Chip("${def.nome} (${def.attr.uppercase()})", def.nome in selecionadas, { if(def.nome in selecionadas || selecionadas.size<limite) onToggle(def.nome) }, Primaria) }
    }
}
...
private fun EquipamentosCriacao(p: Personagem, selecionadas:Set<String>, onToggle:(String)->Unit) {
    Painel(titulo="Etapa 9 — Equipamentos") {
        Text("Escolha as armas iniciais que deseja colocar na ficha. Outros itens podem ser adicionados depois no inventário.", color=TextoFraco, style=MaterialTheme.typography.bodySmall)
        ARMAS_LIVRO.forEach { arma ->
            val label="${arma.nome} — Cat. ${arma.categoria}"
            Chip(label, arma.nome in selecionadas, { onToggle(arma.nome) }, Primaria)
        }
