            ...
            item {
                when (etapa) {
                    ...
                    8 -> PericiasCriacao(p = personagem, selecionadas = pericias, onToggle = { nome ->
                        val novoSet = if (nome in pericias) pericias - nome else pericias + nome
                        pericias = novoSet
                        val mapa = personagem.pericias.toMutableMap()
                        mapa[nome] = (mapa[nome] ?: Pericia()).copy(treino = if (nome in novoSet) 5 else 0)
...
private fun PainelPoder(
    ...
    Column(Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
        ...
        Text(poder.descricao, color = TextoFraco, style = MaterialTheme.typography.bodySmall, modifier = Modifier.padding(start = 8.dp))
        ...
        if (poder.trilha.isNotBlank()) {
            Text("Trilha: ${poder.trilha}", color = Acento, style = MaterialTheme.typography.labelSmall, modifier = Modifier.padding(start = 8.dp))
        }
    }
}
...
private fun PericiasCriacao(p: Personagem, selecionadas: Set<String>, onToggle: (String) -> Unit) {
    ...
    Painel(titulo = "Etapa 8 — Perícias") {
        ...
        PERICIAS.forEach { def ->
            ...
            val manual = def.nome in manuais || def.nome in selecionadasManuais
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Chip(
                    ...
                    selecionada || automatica,
                    {
                        if (!automatica) {
                            if (emTroca != null) {
                                if (def.nome != emTroca && def.nome !in automaticas && def.nome !in manuais && selecionadasManuais.size <= limite) {
                                    val mapa = p.pericias.toMutableMap()
                                    mapa[def.nome] = (mapa[def.nome] ?: Pericia()).copy(treino = 5)
                                    onToggle(def.nome)
                                    emTroca = null
                                }
                    ...
                    enabled = !automatica && (emTroca != null || selecionada || selecionadasManuais.size < limite),
                )
                if (manual && !automatica) {
                    TextButton(onClick = { trocarDe(def.nome) }) { Text("Trocar") }
                }
            }
        }
    }
