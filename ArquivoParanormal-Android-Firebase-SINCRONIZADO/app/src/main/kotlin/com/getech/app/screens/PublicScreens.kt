...
fun PublicPortalScreen(
    ...
    TechScaffold(
        ...
        actions = {
            ...
            TextButton(onClick = onProfile) { Text("Perfil", color = MaterialTheme.colorScheme.primary) }
...
@Composable
fun ContactScreen(repo: LocalRepository, onBack: () -> Unit) {
    var name by remember { mutableStateOf(repo.session()?.name ?: "") }
    var email by remember { mutableStateOf(repo.session()?.email ?: "") }
    var message by remember { mutableStateOf("") }
    var feedback by remember { mutableStateOf("") }
    TechScaffold("Contato", onBack = onBack) { pad ->
        LazyColumn(Modifier.padding(pad).padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item {
                TechCard {
                    Text("Fale com a GeTech", style = MaterialTheme.typography.headlineMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Envie uma solicitação pelo próprio aplicativo. O registro fica disponível na Caixa de Mensagens do Gestor.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                    ...
                    OutlinedTextField(message, { message = it }, Modifier.fillMaxWidth(), label = { Text("Mensagem") }, minLines = 5)
                    if (feedback.isNotBlank()) Text(feedback, color = if (feedback.startsWith("Enviado")) Green else Red)
                    PrimaryButton("Enviar mensagem", Modifier.fillMaxWidth()) {
                        feedback = if (repo.addMessage(name, email, message, "Contato")) {
                            message = ""
                            "Enviado com sucesso."
                        } else "Preencha nome, e-mail válido e mensagem."
                    }
                }
            }
        }
...
@Composable
fun ProfileScreen(repo: LocalRepository, onBack: () -> Unit) {
    val user = repo.session()
    var name by remember { mutableStateOf(user?.name ?: "") }
    var feedback by remember { mutableStateOf("") }
    TechScaffold("Meu Perfil", onBack = onBack) { pad ->
        Column(Modifier.padding(pad).padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            TechCard {
                Text("Perfil GeTech", style = MaterialTheme.typography.headlineMedium)
                Text("${user?.email ?: ""}", color = MaterialTheme.colorScheme.onSurfaceVariant)
                StatusBadge((user?.role ?: "cliente").uppercase())
                OutlinedTextField(name, { name = it }, Modifier.fillMaxWidth(), label = { Text("Nome") }, singleLine = true)
                if (feedback.isNotBlank()) Text(feedback, color = if (feedback.startsWith("Perfil")) Green else Red)
                PrimaryButton("Salvar alterações", Modifier.fillMaxWidth()) {
                    feedback = if (repo.updateProfileName(name)) "Perfil atualizado." else "Informe um nome válido."
                }
            }
        }
    }
