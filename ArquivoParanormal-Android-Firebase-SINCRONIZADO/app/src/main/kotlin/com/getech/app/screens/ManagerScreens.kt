...
fun ManagerModuleScreen(module:Module,repo:LocalRepository,onBack:()->Unit){
    ...
    TechScaffold(module.title,onBack=onBack){pad->LazyColumn(Modifier.padding(pad).padding(18.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
        ...
        when(module.route){
            ...
            "logs"->{items(tickets.take(20)){t->TechCard(title="${SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault()).format(Date(t.createdAt))} • ${t.name}",icon="📝"){Text("E-mail: ${t.email}",color=TextSecondary);Text(t.problem,color=TextPrimary)}}}
