...
@Composable
fun ClientDashboard(repo:LocalRepository,onChat:()->Unit,onAR:()->Unit,onPublic:(String)->Unit,onLogout:()->Unit,onBack:()->Unit,onTheme:()->Unit){
 val u=repo.session()
 TechScaffold("Área do Cliente",onBack=onBack,actions={
  ...
  LazyColumn(Modifier.padding(pad).padding(18.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
   ...
   item{SectionTitle("Portal GeTech");Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){SecondaryButton("Funcionalidades",Modifier.weight(1f)){onPublic("funcionalidades")};SecondaryButton("FAQ",Modifier.weight(1f)){onPublic("faq")}}}
   item{Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){SecondaryButton("Contato",Modifier.weight(1f)){onPublic("contato")};SecondaryButton("Privacidade",Modifier.weight(1f)){onPublic("privacidade")}}}
