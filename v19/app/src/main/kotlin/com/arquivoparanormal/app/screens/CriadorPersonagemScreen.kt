...
private fun PoderesParanormaisCriacao(
    ...
    slotAprenderRitualDialog?.let { slot ->
        ...
        val rituaisElegiveis = RITUAIS_COMPLETOS.filter { r ->
            val c = when (r.circulo) { "1º" -> 1; "2º" -> 2; "3º" -> 3; "4º" -> 4; else -> 99 }
            c <= maxCirculoPermitido
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (etapaTroca == 0) {
                        ...
                        p.rituais.filterNot { it.nome in automaticos }.forEach { r ->
                            Button(
                                onClick = { ritualSelecionadoAntigo = r; etapaTroca = 2 },
                                modifier = Modifier.fillMaxWidth(),
                            ) { Text("${r.nome} · ${r.circulo} · ${r.elemento}") }
                        }
                        if (p.rituais.none { it.nome !in automaticos }) {
                        ...
                        Text(
                            ...
                            else "NEX ${p.nex}% permite escolher um ritual de até $maxCirculoPermitidoº círculo.",
                            color = TextoFraco,
                        )
                        rituaisElegiveis
                                ...
                                Button(
                                    onClick = {
                                        val novoRitual = Ritual(
                                            ...
                                            efeitoCampo = r.efeitoCampo, duracao = r.duracao, afinidade = r.afinidade,
            ...
            val adicionados = faltantes.mapNotNull { nome ->
                RITUAIS_COMPLETOS.firstOrNull { it.nome == nome }?.let { def ->
                    Ritual(
                        ...
                        efeitoCampo = def.efeitoCampo, duracao = def.duracao, afinidade = def.afinidade,
