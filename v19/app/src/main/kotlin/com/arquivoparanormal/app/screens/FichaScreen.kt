    ...
    LazyColumn(
        ...
        item {
            ...
            Painel(titulo = "Resistências a dano") {
                Text("Automático = equipamentos equipados + efeitos ativos. Outros = ajuste manual. As resistências se acumulam quando as fontes permitem.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                RESISTENCIAS.chunked(2).forEach { linha ->
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        linha.forEach { r ->
                            val auto = calc.resistenciasAutomaticas[r] ?: 0
                            val manual = p.resistencias[r] ?: 0
                            val total = auto + manual
                            Campo(r, Modifier.weight(1f)) {
                                Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                                    Text("Total $total", color = if (total > 0) Acento else TextoClaro)
                                    Text("Auto $auto", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                                    ...
                                    Numero(manual) { upd(p.copy(resistencias = p.resistencias + (r to it.coerceAtLeast(0)))) }
                                }
                            }
                        }
                    }
                }
                Text("Ex.: RD 5 balístico reduz 5 do dano balístico; não reduz corte, impacto etc.", color = TextoFraco, style = MaterialTheme.typography.bodySmall)
                ...
                armaCatalogo?.let { def ->
                    Painel(
                        ...
                        Campo("Descrição") {
                            ...
                            TextButton(onClick = { detalhesCatalogoAbertos = true }) { Text("Ver detalhes") }
                        }
                        if (detalhesCatalogoAbertos) {
                            AlertDialog(
                                onDismissRequest = { detalhesCatalogoAbertos = false },
                                title = { Text("${def.nome} — detalhes") },
                                text = {
                                    androidx.compose.foundation.layout.Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                        Text("Execução: ${def.execucao}", color = TextoClaro)
                                        Text("Alcance: ${def.alcance}", color = TextoClaro)
                                        if (def.alvo.isNotBlank()) Text("Alvo: ${def.alvo}", color = TextoClaro)
                                        if (def.area.isNotBlank()) Text("Área: ${def.area}", color = TextoClaro)
                                        if (def.efeito.isNotBlank()) Text("Efeito: ${def.efeito}", color = TextoClaro)
                                        if (def.duracao.isNotBlank()) Text("Duração: ${def.duracao}", color = TextoClaro)
                                        if (def.resistencia.isNotBlank()) Text("Resistência: ${def.resistencia} · DT ${calcularFicha(p).dtRituais}", color = TextoClaro)
                                        Text(def.descricao, color = TextoClaro)
                                        if (def.discenteDescricao.isNotBlank()) {
                                            Text("Discente (+${def.discenteExtraPE} PE)", color = Acento)
                                            Text(def.discenteDescricao, color = TextoClaro)
                                            if (def.requisitoDiscente.isNotBlank()) Text("Requisito: ${def.requisitoDiscente}", color = TextoFraco)
                                        }
                                        if (def.verdadeiroDescricao.isNotBlank()) {
                                            Text("Verdadeira (+${def.verdadeiroExtraPE} PE)", color = Acento)
                                            Text(def.verdadeiroDescricao, color = TextoClaro)
                                            if (def.requisitoVerdadeiro.isNotBlank()) Text("Requisito: ${def.requisitoVerdadeiro}", color = TextoFraco)
                                        }
                                    }
                                },
                                confirmButton = { TextButton(onClick = { detalhesCatalogoAbertos = false }) { Text("Fechar") } },
                            )
                        }
                        Button(
                            enabled = ritualProntoParaAdicionar,
                            onClick = {
                                upd(
                                    p.copy(
                                        armas = p.armas + Arma(
            ...
            Painel(
                ...
                itensVisiveis.forEach { i ->
                    ...
                    if (i.tipoInventario == "Proteções") {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Campo("Defesa do item", Modifier.weight(1f)) { Numero(i.defesaBonus) { set(i.copy(defesaBonus = it)) } }
                            Campo("RD geral", Modifier.weight(1f)) { Numero(i.resistencias["Geral"] ?: 0) { set(i.copy(resistencias = i.resistencias + ("Geral" to it.coerceAtLeast(0)))) } }
                        }
                        Text("Equipado = entra automaticamente na Defesa e nas Resistências.", color = TextoFraco, style = MaterialTheme.typography.labelSmall)
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        item {
            ...
            val opcoesRituais = RITUAIS_COMPLETOS
            ...
            val ritualCatalogoBase = RITUAIS_COMPLETOS.firstOrNull {
